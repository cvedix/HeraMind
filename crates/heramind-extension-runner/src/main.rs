...
//! It communicates with the main HeraMind process via stdin/stdout using
//! the IPC protocol.
//!
...
//!
//! ```bash
//! heramind-extension-runner --extension-path /path/to/extension.dylib
//! heramind-extension-runner --extension-path /path/to/extension.wasm
//! ```
//!
//! # Protocol
//!
...
#[derive(Parser, Debug)]
#[command(name = "heramind-extension-runner")]
#[command(about = "Run a HeraMind extension in isolated mode")]
struct Args {
...
unsafe extern "C" fn push_output_raw_writer(
    ...
    let payload = heramind_extension_sdk::encode_segmented_payload(&header_bytes, bytes);
...
unsafe extern "C" fn push_output_writer(data: *const u8, len: usize) -> i32 {
    ...
    // (see heramind_extension_sdk::parse_response_payload).
    let data: Vec<u8> = {
    ...
    let payload = heramind_extension_sdk::encode_segmented_payload(&header, &data);
...
type RegisterPushWriterFn = unsafe extern "C" fn(heramind_extension_sdk::PushOutputWriterFn) -> i32;
type RegisterPushWriterRawFn =
    unsafe extern "C" fn(heramind_extension_sdk::PushOutputRawWriterFn) -> i32;
...
impl NativeExtensionBridge {
    fn load(path: &Path) -> Result<(Self, ExtensionDescriptor), String> {
        ...
        let abi_version = Self::load_symbol::<unsafe extern "C" fn() -> u32>(
            &library,
            b"heramind_extension_abi_version\0",
        )?;
        ...
        let bridge = Self {
            free_string: Self::load_symbol(&library, b"heramind_extension_free_string\0")?,
            descriptor_json: Self::load_symbol(&library, b"heramind_extension_descriptor_json\0")?,
            set_capability_bridge: Self::load_optional_symbol(
                &library,
                b"heramind_extension_set_capability_bridge\0",
            ),
            register_push_writer: Self::load_optional_symbol(
                &library,
                b"heramind_extension_register_push_writer\0",
            ),
            register_push_writer_raw: Self::load_optional_symbol(
                &library,
                b"heramind_extension_register_push_writer_raw\0",
            ),
            execute_command_json: Self::load_symbol(
                &library,
                b"heramind_extension_execute_command_json\0",
            )?,
            configure_json: Self::load_optional_symbol(
                &library,
                b"heramind_extension_configure_json\0",
            ),
            produce_metrics_json: Self::load_symbol(
                &library,
                b"heramind_extension_produce_metrics_json\0",
            )?,
            health_check_json: Self::load_optional_symbol(
                &library,
                b"heramind_extension_health_check_json\0",
            ),
            stats_json: Self::load_optional_symbol(&library, b"heramind_extension_stats_json\0"),
            event_subscriptions_json: Self::load_optional_symbol(
                &library,
                b"heramind_extension_event_subscriptions_json\0",
            ),
            stream_capability_json: Self::load_optional_symbol(
                &library,
                b"heramind_extension_stream_capability_json\0",
            ),
            init_session_json: Self::load_optional_symbol(
                &library,
                b"heramind_extension_init_session_json\0",
            ),
            process_session_chunk_json: Self::load_optional_symbol(
                &library,
                b"heramind_extension_process_session_chunk_json\0",
            ),
            close_session_json: Self::load_optional_symbol(
                &library,
                b"heramind_extension_close_session_json\0",
            ),
            process_chunk_json: Self::load_optional_symbol(
                &library,
                b"heramind_extension_process_chunk_json\0",
            ),
            start_push_json: Self::load_optional_symbol(
                &library,
                b"heramind_extension_start_push_json\0",
            ),
            stop_push_json: Self::load_optional_symbol(
                &library,
                b"heramind_extension_stop_push_json\0",
            ),
            handle_event_json: Self::load_optional_symbol(
                &library,
                b"heramind_extension_handle_event_json\0",
            ),
            _library: library,
        };
    ...
    fn load_symbol<T: Copy>(library: &libloading::Library, name: &[u8]) -> Result<T, String> {
        let symbol: libloading::Symbol<T> = unsafe {
    ...
    fn load_optional_symbol<T: Copy>(library: &libloading::Library, name: &[u8]) -> Option<T> {
        Self::load_symbol(library, name).ok()
    }
}
...
impl WasmRuntime {
    fn new(path: &PathBuf, module_name: String) -> Result<Self, String> {
        ...
        // (HERAMIND_WASM_MEMORY_MB env dropped; re-add via ResourceLimiter).
        ...
        let max_wasm_size = std::env::var("HERAMIND_WASM_MAX_SIZE_MB")
            .ok()
    ...
    async fn get_descriptor_async(
        ...
        // Add heramind host functions for capability invocation
        add_heramind_to_linker(&mut linker)?;
        ...
        store
            .set_fuel(
                std::env::var("HERAMIND_WASM_FUEL")
                    .ok()
                    .and_then(|v| v.parse::<u64>().ok())
                    .unwrap_or(1_000_000),
            )
...
/// Add heramind host functions to the linker
///
...
fn add_heramind_to_linker(linker: &mut Linker<HostState>) -> Result<(), String> {
        ...
        match message {
            ...
            IpcMessage::EventPush {
                ...
                if let Some(extension) = &self.extension {
                    ...
                    heramind_extension_sdk::capabilities::event::call_event_handler(
                        &event_type,
                        &payload,
                    );
                }
...
// Worker-thread count: configurable via HERAMIND_RUNNER_WORKERS, default 4
...
fn runner_worker_threads() -> usize {
    std::env::var("HERAMIND_RUNNER_WORKERS")
        .ok()
...
async fn async_main() {
    ...
    debug!("HeraMind Extension Runner starting");
    let _ = std::io::stderr().flush();
...
mod tests {
    #[test]
    fn native_symbols_accept_both_sdk_namespaces() {
        let tmp = tempfile::tempdir().unwrap();
        let source = tmp.path().join("symbols.rs");
        std::fs::write(
            &source,
            r#"
            #[no_mangle] pub extern "C" fn neomind_extension_abi_version() -> u32 { 3 }
            #[no_mangle] pub extern "C" fn heramind_extension_preferred() -> u32 { 24 }
            #[no_mangle] pub extern "C" fn neomind_extension_preferred() -> u32 { 11 }
        "#,
        )
        .unwrap();
        let library_path = tmp
            .path()
            .join(format!("symbols{}", std::env::consts::DLL_SUFFIX));
        ...
        let abi: unsafe extern "C" fn() -> u32 = super::NativeExtensionBridge::load_symbol(
            &library,
            b"heramind_extension_abi_version\0",
        )
        .unwrap();
        let preferred: unsafe extern "C" fn() -> u32 =
            super::NativeExtensionBridge::load_symbol(&library, b"heramind_extension_preferred\0")
                .unwrap();
        assert_eq!(unsafe { abi() }, 3);
        assert_eq!(unsafe { preferred() }, 24);
        assert!(
            super::NativeExtensionBridge::load_optional_symbol::<unsafe extern "C" fn()>(
                &library,
                b"heramind_extension_missing\0"
            )
            .is_none()
        );
    }
