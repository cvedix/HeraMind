...
use heramind_extension_sdk::{IpcMessage, IpcResponse};
...
pub(crate) fn start_stdin_reader() -> std::thread::JoinHandle<()> {
    std::thread::spawn(|| {
        ...
        loop {
            ...
            let max_size = std::env::var("HERAMIND_IPC_MAX_SIZE")
                .ok()
...
mod tests {
    ...
    fn test_ipc_response_metrics_roundtrip() {
        use heramind_extension_sdk::{ExtensionMetricValue, MetricValue};
