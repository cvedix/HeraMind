...
use heramind_devices::adapters::webhook::WebhookAdapter;
...
async fn test_webhook_convert_binary_to_url() {
    ...
    let temp_dir = tempfile::tempdir().unwrap();
    let config = heramind_devices::adapters::webhook::WebhookAdapterConfig::new("test-webhook");
    let device_registry = std::sync::Arc::new(heramind_devices::registry::DeviceRegistry::new());
