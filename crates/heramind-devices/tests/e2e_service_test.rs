...
use heramind_devices::mdl::MetricDataType;
use heramind_devices::mdl_format::{CommandDefinition, MetricDefinition, ParameterDefinition};
...
impl DeviceAdapter for TestAdapter {
    ...
    fn subscribe(&self) -> Pin<Box<dyn Stream<Item = heramind_devices::DeviceEvent> + Send + '_>> {
    ...
    fn connection_status(&self) -> heramind_devices::adapter::ConnectionStatus {
        if self.is_running() {
            heramind_devices::adapter::ConnectionStatus::Connected
        } else {
            heramind_devices::adapter::ConnectionStatus::Disconnected
        }
    }
