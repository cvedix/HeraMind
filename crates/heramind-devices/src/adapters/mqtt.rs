//! MQTT device adapter for HeraMind event-driven architecture.
//!
...
impl MqttAdapterConfig {
    ...
    pub fn new(name: impl Into<String>, broker: impl Into<String>) -> Self {
        let mqtt_config = MqttConfig::new(broker, "heramind");
        Self {
            name: name.into(),
            mqtt: mqtt_config,
...
impl MqttAdapter {
    ...
    pub async fn add_broker(
        ...
        // Build MQTT options
        let client_id = format!("heramind-{}-{}", broker_id, Uuid::new_v4());
    ...
    pub async fn add_broker_with_tls(
        ...
        // Build MQTT options
        let mqtt_client_id =
            client_id.unwrap_or_else(|| format!("heramind-{}-{}", broker_id, Uuid::new_v4()));
    ...
    async fn handle_mqtt_notification(
        ...
        match notification {
            rumqttc::Event::Incoming(rumqttc::Packet::Publish(publish)) => {
                ...
                //
                // Skip internal HeraMind clients (e.g. `heramind-external-{id}` —
                ...
                if topic.starts_with("$SYS/brokers/") {
                        ...
                        parse_sys_presence_topic(&topic)
                    {
                        if sys_client_id.starts_with("heramind-") {
                            ...
                            if is_online {
                                bus.publish(HeraMindEvent::DeviceTransportOnline {
                                ...
                                bus.publish(HeraMindEvent::DeviceTransportOffline {
                                    device_id: sys_client_id.clone(),
                                    client_id: sys_client_id.clone(),
                                    reason: None,
                                    timestamp: ts,
                ...
                if !is_standard_uplink {
                    ...
                    if let Some(ref device_id) = device_id_opt {
                        ...
                        // Publish DeviceDiscovered event via DeviceEvent::Discovery for auto-onboarding
                        {
                            ...
                            if let Some(bus) = event_bus {
                                bus.publish(HeraMindEvent::DeviceDiscovered {
                                    device_id: auto_device_id,
    ...
    tokio::spawn(async move {
        ...
        while let Some(event) = rx.next().await {
            if let Some(device_id) = event.device_id() {
                ...
                let heramind_event = event.clone().to_heramind_event();
...
pub async fn test_mqtt_connection(
    ...
    let client_id = format!("heramind-test-{}", uuid::Uuid::new_v4());
