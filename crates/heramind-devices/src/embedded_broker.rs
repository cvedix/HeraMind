...
//! The broker runs in the same process as HeraMind, eliminating the need
...
use heramind_core::{EventBus, HeraMindEvent};

/// Topic-to-device-id resolver used by `DevicePresenceHook` to learn which
/// HeraMind `device_id` corresponds to an MQTT `client_id`.
///
/// Devices don't always set their MQTT `client_id` equal to their HeraMind
...
struct HeraMindAuthHandler {
    auth_enabled: Arc<AtomicBool>,
    credential_validator: CredentialValidatorFn,
}

#[async_trait]
impl rmqtt::hook::Handler for HeraMindAuthHandler {
    async fn hook(
        &self,
            ...
            if let (Some(uname), Some(pwd)) = (username, password) {
                ...
                if let Ok(pwd_str) = std::str::from_utf8(pwd_bytes) {
                    ...
                    if (self.credential_validator)(uname_str, pwd_str) {
                        let is_superuser = uname_str == "__heramind_internal__";
                        tracing::debug!(
...
/// to the HeraMind EventBus whenever an MQTT client connects or disconnects
...
struct DevicePresenceHook {
    ...
    /// Shared cache: MQTT client_id → HeraMind device_id, learned from
...
impl DevicePresenceHook {
    ...
    /// Resolve an rmqtt client_id to a HeraMind device_id.
    ///
    /// Priority:
    ...
    /// True if this client_id belongs to a HeraMind-internal MQTT client
    ...
    ///
    /// The `heramind-` prefix is reserved for internal use. User-registered
    ...
    fn is_internal_client(client_id: &rmqtt::types::ClientId) -> bool {
        client_id.to_string().starts_with("heramind-")
    }
}
    ...
    async fn hook(
        ...
        match param {
            rmqtt::hook::Parameter::ClientConnected(session) => {
                ...
                self.event_bus
                    .publish(HeraMindEvent::DeviceTransportOnline {
                        device_id,
            ...
            rmqtt::hook::Parameter::ClientDisconnected(session, reason) => {
                ...
                self.event_bus
                    .publish(HeraMindEvent::DeviceTransportOffline {
                        device_id,
                        client_id: client_id_str,
                        reason: reason_str,
                        timestamp: now,
            ...
            rmqtt::hook::Parameter::MessagePublish(_session, from, publish) => {
                ...
                if client_id_str.starts_with("heramind-") {
                    return (true, _acc);
                }
...
pub struct EmbeddedBroker {
    ...
    /// to HeraMind device_id (necessary when devices use a client_id
...
impl EmbeddedBroker {
    ...
    /// mapping from MQTT client_id to HeraMind device_id. This is necessary
    ...
    pub async fn start(&self) -> Result<(), EmbeddedBrokerError> {
        ...
        let auth_handler = HeraMindAuthHandler {
        ...
        tracing::info!(
            "MQTT Broker Listening on heramind-broker {} (auth_enabled={})",
            addr,
