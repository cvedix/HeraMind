//! Device adapter interface for HeraMind event-driven architecture.
//!
//! This module defines the adapter pattern for integrating various data sources
//! (MQTT, HASS, HTTP, etc.) into the HeraMind platform through a unified interface.
...
/// the HeraMind system. They are typically converted to HeraMindEvent and
...
impl DeviceEvent {
    ...
    /// Convert to HeraMindEvent.
    pub fn to_heramind_event(self) -> HeraMindEvent {
        match self {
            Self::Metric {
                device_id,
                metric,
            ...
            Self::State {
                ...
                match new_state {
                    ConnectionStatus::Connected => HeraMindEvent::DeviceOnline {
                        device_id,
            ...
            Self::Discovery { device } => {
                ...
                HeraMindEvent::DeviceDiscovered {
            ...
            Self::CommandResult {
                ...
                result,
                timestamp,
            } => HeraMindEvent::DeviceCommandResult {
                device_id,
                command,
                success,
...
fn convert_metric_value(value: MetricValue) -> heramind_core::MetricValue {
    use serde_json::json;
    match value {
        MetricValue::Integer(v) => heramind_core::MetricValue::Integer(v),
        MetricValue::Float(v) => heramind_core::MetricValue::Float(v),
        MetricValue::String(v) => heramind_core::MetricValue::String(v),
        MetricValue::Boolean(v) => heramind_core::MetricValue::Boolean(v),
            ...
            while running.load(std::sync::atomic::Ordering::Relaxed) {
                match rx.next().await {
                    Some(event) => {
                        ...
                        let heramind_event = event.to_heramind_event();
                        event_bus.publish_with_source(heramind_event, source).await;
                    }
                    None => break,
                }
            }
...
mod tests {
    ...
    #[test]
    fn test_device_event_to_heramind() {
        let event = DeviceEvent::Metric {
        ...
        let heramind = event.to_heramind_event();
        assert!(matches!(heramind, HeraMindEvent::DeviceMetric { .. }));
    }

    #[test]
    fn test_device_event_state_to_heramind() {
        ...
        let heramind = event.to_heramind_event();
        assert!(matches!(heramind, HeraMindEvent::DeviceOnline { .. }));
    }
