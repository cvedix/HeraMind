...
use heramind_storage::device_registry::{
...
impl DeviceService {
    ...
    pub async fn start(&self) {
        ...
        tokio::spawn(async move {
            ...
            let filter = |event: &heramind_core::HeraMindEvent| -> bool {
                matches!(
                    event,
                    heramind_core::HeraMindEvent::DeviceOnline { .. }
                        | heramind_core::HeraMindEvent::DeviceOffline { .. }
                        | heramind_core::HeraMindEvent::DeviceTransportOnline { .. }
                        | heramind_core::HeraMindEvent::DeviceTransportOffline { .. }
            ...
            while let Some((event, _)) = rx.recv().await {
                match event {
                    heramind_core::HeraMindEvent::DeviceOnline { device_id, .. } => {
                    ...
                    heramind_core::HeraMindEvent::DeviceOffline { device_id, .. } => {
                        let mut status = device_status.write().await;
                        let entry = status.entry(device_id.clone()).or_default();
                    ...
                    heramind_core::HeraMindEvent::DeviceMetric {
                        ...
                        if !heramind_core::HeraMindEvent::is_virtual_device_metric(
                            is_virtual, &metric,
                        ) {
                            let mut status = device_status.write().await;
                            let entry = status.entry(device_id.clone()).or_default();
                            ...
                            if entry.status == ConnectionStatus::Disconnected {
                                ...
                                event_bus_for_publish
                                    .publish(heramind_core::HeraMindEvent::DeviceOnline {
                        ...
                        if let Some(storage) = ts_storage.as_ref() {
                            ...
                            let metric_value: MetricValue = match &value {
                                heramind_core::MetricValue::Integer(i) => MetricValue::Integer(*i),
                                heramind_core::MetricValue::Float(f) => MetricValue::Float(*f),
                                heramind_core::MetricValue::String(s) => {
                                    MetricValue::String(s.clone())
                                }
                                heramind_core::MetricValue::Boolean(b) => MetricValue::Boolean(*b),
                                heramind_core::MetricValue::Json(j) => {
            ...
            *heartbeat_running.write().await = true;
            ...
            loop {
                ...
                for (device_id, stale_last_seen) in stale_devices {
                    ...
                    let _ = event_bus
                        .publish(heramind_core::HeraMindEvent::DeviceOffline {
                            device_id: device_id.clone(),
                            reason: Some(format!(
...
impl DeviceService {
    ...
    pub async fn register_template(&self, template: DeviceTypeTemplate) -> Result<(), DeviceError> {
        ...
        tokio::spawn({
            ...
            async move {
                let _ = event_bus
                    .publish(heramind_core::HeraMindEvent::Custom {
...
mod tests {
    ...
    fn test_resolve_command_image_urls_resolves_api_images() {
        ...
        let temp =
            std::env::temp_dir().join(format!("heramind_test_cmd_img_{}", std::process::id()));
    ...
    fn test_resolve_command_image_urls_missing_falls_back() {
        ...
        let temp =
            std::env::temp_dir().join(format!("heramind_test_cmd_empty_{}", std::process::id()));
    ...
    fn test_purge_device_images_removes_device_dir_leaves_siblings() {
        ...
        let temp = std::env::temp_dir().join(format!("heramind_test_purge_{}", std::process::id()));
    ...
    fn test_purge_device_images_rejects_traversal_device_id() {
        ...
        let temp =
            std::env::temp_dir().join(format!("heramind_test_purge_trav_{}", std::process::id()));
        let images = temp.join("images");
    ...
    fn test_purge_device_images_missing_dir_is_noop() {
        let temp =
            std::env::temp_dir().join(format!("heramind_test_purge_noop_{}", std::process::id()));
        std::fs::create_dir_all(&temp).unwrap();
