...
pub struct CliResponse {
    ...
    /// e.g. "Run 'heramind device list' to see available devices"
    #[serde(skip_serializing_if = "Option::is_none")]
...
mod tests {
    ...
    fn test_cli_response_error_with_suggestion() {
        let response = CliResponse::error_with_suggestion(
            "Device not found",
            "NOT_FOUND",
            "Run 'heramind device list' to see available devices",
        );
        assert!(!response.success);
        assert_eq!(response.error, Some("Device not found".to_string()));
        assert_eq!(
            response.suggestion,
            Some("Run 'heramind device list' to see available devices".to_string())
        );
    }
    ...
    fn test_cli_response_success_with_meta() {
        ...
        let meta = BuildMeta {
            ...
            undo_command: "heramind device delete 456".to_string(),
        };
    ...
    fn test_build_meta_serialization() {
        let meta = BuildMeta {
            ...
            undo_command: "heramind dashboard update 789".to_string(),
        };
        ...
        assert_eq!(deserialized.undo_command, "heramind dashboard update 789");
    }
