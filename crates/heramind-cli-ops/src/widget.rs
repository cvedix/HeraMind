...
/// The user (or AI) then edits the files and installs via `heramind widget install`.
pub fn create_widget(
    ...
    let global_name = format!("HeraMind{}", to_pascal_case(&widget_id));
...
/// Generate IIFE bundle.js template following HeraMind-Dashboard-Components standards.
...
// Generated by heramind widget create
// HeraMind Dashboard Component (IIFE format)
//
// Runtime: uses window.React and window.jsxRuntime (provided by HeraMind)
...
{component_body}
  ...
  // Assign to global for HeraMind component registry
...
/// `heramind_core::dashboard::BUILTIN_WIDGET_TYPES`, mirrored from
/// `Renderers.tsx::builtInTypes`). Full manifest for an installed widget
/// is available via `heramind widget get <id>`.
pub async fn list_widgets(client: &ApiClient) -> Result<CliResponse> {
    ...
    // `heramind_core::dashboard::BUILTIN_WIDGET_TYPES` on the server) — the
...
pub async fn install_widget_file(client: &ApiClient, path: &str) -> Result<CliResponse> {
            ...
            return Ok(CliResponse::error(
                format!(
                    "Expected a directory or .zip file, got: {}. \
                     Usage: heramind widget install <directory> OR heramind widget install <file.zip>",
                    path
                ),
                "INVALID_PATH_TYPE",
            ));
    ...
    let meta = BuildMeta {
        ...
        undo_command: format!("heramind widget uninstall {}", widget_id),
    };
...
pub async fn install_widget_market(
    ...
    let meta = BuildMeta {
        ...
        entity_name: Some(widget_name),
        undo_command: format!("heramind widget uninstall {}", installed_id),
    };
