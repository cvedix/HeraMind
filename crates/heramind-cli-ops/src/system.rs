...
pub async fn system_info(client: &ApiClient) -> Result<CliResponse> {
    ...
    // consumed by the server. It only knows its own `HERAMIND_API_BASE` (or the
    // localhost default). Surface the source so users know whether to trust the
    // URL: `env` = operator set HERAMIND_API_BASE explicitly; `fallback` = the
    ...
    let mut info = json!({
        "mqtt": {
            ...
            "discovery_topic": "heramind/discovery/#",
        },
        "network": {
            "server_ip": server_ip,
            "wifi_ssid": wifi_ssid,
        ...
        "device_connection": {
            "mqtt": {
                ...
                "auto_discovery": true,
                "discovery_prefix": "heramind/discovery",
                "tls": tls_enabled,
