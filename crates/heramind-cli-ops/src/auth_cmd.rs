//! `heramind login` / `logout` / `whoami` command implementations.
//!
//! Provides a `gh auth login`-style credential bootstrap so that the CLI works
//! from any CWD after a single `heramind login`. Credentials are stored in the
//! platform config dir (`dirs::config_dir()/heramind/api_key`) with 0600 perms.
            ...
            return Ok(CliResponse::error_with_suggestion(
                ...
                format!(
                    "Ensure --data-dir points to the correct server data directory (resolved: {}). \
                     If the issue persists, restart the server with: heramind serve",
                    resolved_dir
                ),
            ));
        }
...
/// `heramind logout` — remove the CLI credential file.
...
/// `heramind whoami` — validate the current key against the server.
pub async fn run_whoami() -> Result<CliResponse> {
    // Determine current key (same resolution as ApiClient)
    let key = std::env::var("HERAMIND_API_KEY")
        .ok()
    ...
    let key = match key {
        ...
        None => {
            return Ok(CliResponse::error_with_suggestion(
                "No API key found. Log in first with: heramind login",
                "NO_KEY",
                "Run: heramind login",
            ));
        }
        ...
        Err(e) => {
            ...
            if msg.contains("401") {
                Ok(CliResponse::error_with_suggestion(
                    ...
                    "INVALID_KEY",
                    "Refresh with: heramind login --force",
                ))
            } else {
                Ok(CliResponse::error_with_suggestion(
                    format!("Cannot reach server at {}: {}", base_url, msg),
                    "SERVER_UNREACHABLE",
                    "Ensure the server is running: heramind serve",
                ))
            }
        }
    }
...
fn key_source() -> &'static str {
    if std::env::var("HERAMIND_API_KEY").is_ok() {
        "env (HERAMIND_API_KEY)"
    } else if auto_auth::read_logged_in_key().is_some() {
        "credential file (heramind login)"
    } else {
        "redb auto-auth"
    }
}
