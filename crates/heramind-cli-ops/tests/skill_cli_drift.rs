...
//!   2. For each command, attempt `Args::try_parse_from(["heramind", domain, action, ...])`.
//!   3. If parse fails, surface the offending skill + command in the error.
//!
//! To run locally:
//!     cargo test -p heramind-cli-ops --test skill_cli_drift
//!
...
const SKILLS_DIR_RELATIVE: &str = "crates/heramind-agent/src/skills/builtins";
const MANIFEST_ENV_VAR: &str = "HERAMIND_SKILL_MANIFEST";
...
fn try_parse_command(parts: &[String]) -> Result<(), String> {
    ...
    let mut argv: Vec<String> = vec!["heramind".to_string()];
    argv.extend(parts.iter().cloned());

    // Skills reference commands like `heramind device get <ID>` — we substitute
...
/// Reduce `heramind device get device-001` → `heramind device get <dummy>`
...
fn strip_id_arguments(argv: &[String]) -> Vec<String> {
    ...
    if NESTED_DOMAINS.contains(&(domain.as_str(), action.as_str())) {
        if argv.len() >= 4 {
            // Keep `heramind <domain> <action> <sub>` and drop the rest (IDs).
            return argv[..4].to_vec();
        }
        return argv.to_vec();
    }

    // Otherwise keep only the first 3 tokens (heramind <domain> <action>).
