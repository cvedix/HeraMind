...
use heramind_core::{config::agent_env_vars, llm::backend::LlmRuntime, Message};
...
impl Agent {
        ...
        let (llm, model_name) = match backend {
            ...
            LlmBackend::Qwen {
                ...
                let runtime = CloudRuntime::new(config).map_err(
                    |e: heramind_core::llm::backend::LlmError| HeraMindError::llm(e.to_string()),
                )?;
    ...
    fn generate_base_prompt(&self) -> String {
        ...
        prompt.push_str(
            "- Use `shell` tool with `heramind` CLI commands for all platform operations\n",
        );
    ...
    fn try_fast_path(&self, user_message: &str) -> Option<AgentResponse> {
        ...
        let greeting_responses: &[(&str, &str)] = &[
            ("你好", "你好！我是 HeraMind 智能助手，有什么可以帮您？"),
            ("您好", "您好！我是 HeraMind 智能助手，有什么可以帮您？"),
            (
                "hi",
                "Hello! I'm HeraMind, your smart assistant. How can I help you?",
            ),
            ("hello", "Hello! I'm HeraMind, your smart assistant."),
            ("早上好", "早上好！今天有什么可以帮您的？"),
            ("下午好", "下午好！有什么可以帮您的？"),
            ("晚上好", "晚上好！有什么可以帮您的？"),
        ];
    ...
    pub async fn process_multimodal(
        ...
        let mut parts = vec![heramind_core::ContentPart::text(user_message)];
        ...
        for image_data in &images {
            ...
            parts.push(heramind_core::ContentPart::image_base64(
                base64_part,
                mime_type_str,
            ));
        ...
        let user_msg = heramind_core::Message::new(
            heramind_core::MessageRole::User,
        ...
        let core_history: Vec<heramind_core::Message> = history_without_last
            .iter()
            .map(|msg| msg.to_core())
            .collect();
            ...
            .await
        {
            ...
            Err(e) => Err(HeraMindError::Llm(format!("LLM processing failed: {}", e))),
        }
    }
    ...
    async fn process_with_llm(&self, user_message: &str) -> Result<AgentResponse> {
        ...
        if let Some(summary) = context_summary {
            ...
            if shared.last_injected_context_hash != summary_hash {
                ...
                use heramind_core::message::{Content, MessageRole};
        ...
        let chat_response = self
            ...
            .await
            .map_err(|e| super::error::HeraMindError::Llm(e.to_string()))?;
...
END"#
    ...
    async fn build_execution_batches(&self, tool_calls: &[ToolCall]) -> Vec<Vec<ToolCall>> {
        ...
        let mut relationships: HashMap<String, heramind_core::tools::ToolRelationships> =
            HashMap::new();
        for tool_call in tool_calls {
            let real_name = self.resolve_tool_name(&tool_call.name);
    ...
    async fn execute_tool(&self, name: &str, arguments: &Value) -> Result<String> {
        ...
        // to a CLI command string that ShellTool expects: {"command": "heramind <domain> ..."}
        ...
        Err(super::error::HeraMindError::Tool(format!(
...
fn apply_cloud_capabilities(
    ...
    capabilities: Option<heramind_core::BackendCapabilities>,
) -> CloudRuntime {
    if let Some(caps) = capabilities {
        tracing::debug!(
            multimodal = %caps.multimodal,
