...
///
/// The default values are synchronized with `heramind_core::llm::backend::StreamConfig`
...
pub async fn process_stream_events_with_safeguards(
    ...
    let stream = stream_result.map_err(|e| HeraMindError::Llm(e.to_string()))?;
    ...
    Ok(Box::pin(async_stream::stream! {
        ...
        'multi_round_loop: loop {
            if tool_iteration_count > 0 {
                ...
                let history_for_llm: Vec<heramind_core::Message> = {
                ...
                let round_stream = match round_stream_result {
                    ...
                    Err(e) => {
                        ...
                        if !all_round_tool_results.is_empty() {
                            ...
                            let summary_history: Vec<heramind_core::Message> = {
                                let state_guard = internal_state.read().await;
            ...
            if tool_calls_detected {
                ...
                if is_degenerate_fence_only_output(&raw_response) {
                    ...
                    let state_guard = internal_state.read().await;
                    let retry_history: Vec<heramind_core::Message> = {
                    ...
                    let retry_user_msg = retry_history.iter()
                        .find(|m| m.role == heramind_core::MessageRole::User)
