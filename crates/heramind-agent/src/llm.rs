...
impl LlmInterface {
    ...
    async fn get_runtime(&self) -> AgentResult<Arc<dyn LlmRuntime>> {
        ...
        Err(HeraMindError::Llm("LLM backend not ready".to_string()))
    }
    ...
    pub async fn set_tool_definitions(
        &self,
        tools: Vec<heramind_core::llm::backend::ToolDefinition>,
    ) {
        use heramind_core::llm::backend::sanitize_tool_name;

        let sanitized: Vec<heramind_core::llm::backend::ToolDefinition> = tools
            .into_iter()
            .map(|mut t| {
    ...
    pub async fn get_tool_definitions(&self) -> Vec<heramind_core::llm::backend::ToolDefinition> {
    ...
    async fn chat_internal(
        ...
        if is_greeting && trimmed.len() < 20 {
            let greeting_response = "您好！我是 HeraMind 智能助手。我可以帮您：\n\
                • 查看设备列表 - 说「列出设备」\n\
                • 查询设备数据 - 说「查询温度」\n\
                • 创建自动化规则 - 说「创建规则」\n\
                • 查看所有规则 - 说「列出规则」";
        ...
        let params = heramind_core::llm::backend::GenerationParams {
        ...
        let messages = if let Some(hist) = history {
            ...
            let history_msgs: Vec<&Message> = hist
                .iter()
                .filter(|msg| msg.role != heramind_core::MessageRole::System)
                .collect();
    ...
    async fn chat_stream_internal_message(
        ...
        let system_prompt = if include_tools {
            ...
            "You are HeraMind, a helpful IoT assistant. Answer questions concisely and accurately."
                .to_string()
        };
        ...
        Ok(Box::pin(async_stream::stream! {
            ...
            while let Some(result) = futures::StreamExt::next(&mut stream).await {
                match result {
                    ...
                    Err(e) => yield Err(HeraMindError::Llm(e.to_string())),
                }
            }
        }))
    }
    ...
    async fn chat_stream_internal(
        ...
        let compact_fallback_system = "You are HeraMind IoT assistant. Summarize results concisely based on the tool results shown.".to_string();
        let compact_fallback_messages = if let Some(hist) = history {
            ...
            let non_system: Vec<&Message> = hist
                .iter()
                .filter(|m| m.role != heramind_core::MessageRole::System)
                .collect();
                    ...
                    Err(LlmError::ContextOverflow { prompt_tokens, max_context }) => {
                        ...
                        if retry_stage <= 2 {
                            ...
                            let retry_input = LlmInput {
                                ...
                                params: heramind_core::llm::backend::GenerationParams {
                                    temperature: Some(eff_temp),
                                    top_p: Some(eff_top_p),
                                    top_k: Some(eff_top_k as u32),
                                    max_tokens: None,
                            ...
                            match llm_retry.generate_stream(retry_input).await {
                                ...
                                Err(e) => {
                                    yield Err(HeraMindError::Llm(format!(
...
mod tests {
    ...
    async fn test_tool_definitions() {
        ...
        let tools = vec![heramind_core::llm::backend::ToolDefinition {
    ...
    fn test_agent_error_display() {
        let err = HeraMindError::Llm("LLM not ready".to_string());
