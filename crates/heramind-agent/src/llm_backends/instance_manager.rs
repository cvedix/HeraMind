...
use dashmap::DashMap;
use heramind_core::llm::backend::{LlmError, LlmInput, LlmRuntime};
use heramind_core::llm::detect_vision_capability;
use heramind_storage::{ConnectionTestResult, LlmBackendInstance, LlmBackendStore, LlmBackendType};
...
///
/// Synchronizes the stored `supports_multimodal` flag with heramind-core's
...
mod tests {
    ...
    fn test_ensure_instance_capabilities_refreshes_stale_text_model() {
        use heramind_storage::{BackendCapabilities, LlmBackendInstance, LlmBackendType};
