...
use chrono_tz::Tz;
use cron::Schedule;
use heramind_storage::{AgentSchedule, AiAgent, ScheduleType};
...
impl AgentScheduler {
    ...
    pub async fn new(config: SchedulerConfig) -> Result<Self, crate::error::HeraMindError> {
    ...
    pub async fn schedule_agent(&self, agent: AiAgent) -> Result<(), crate::error::HeraMindError> {
        let (next_execution, cron_schedule) = self
            .calculate_next_execution(&agent.schedule)
            .await
            .map_err(|e| crate::HeraMindError::Config(format!("Schedule error: {}", e)))?;
