...
impl AgentExecutor {
    pub async fn parse_intent(
        &self,
        user_prompt: &str,
    ) -> AgentResult<heramind_storage::ParsedIntent> {
  ...
  "confidence": 0.9
        ...
        .await
        {
            ...
            Err(_) => {
                ...
                return Err(HeraMindError::Llm(format!(
                    "LLM timeout after {}s",
                    LLM_TIMEOUT_SECS
                )));
            }
        ...
        match llm_result {
            Ok(output) => {
                ...
                serde_json::from_str(json_str).map_err(|_| {
                    HeraMindError::Llm("Failed to parse LLM intent response".to_string())
                })
            }
            Err(_) => Err(HeraMindError::Llm("LLM call failed".to_string())),
        }
    }
    ...
    async fn parse_intent_keywords(
        ...
        {
            (heramind_storage::IntentType::ReportGeneration, 0.8)
        } else if prompt_lower.contains("异常") || prompt_lower.contains("检测") {
            (heramind_storage::IntentType::AnomalyDetection, 0.8)
        } else if prompt_lower.contains("控制") || prompt_lower.contains("开关") {
            (heramind_storage::IntentType::Control, 0.7)
        } else {
            (heramind_storage::IntentType::Monitoring, 0.7)
        };
        ...
        Ok(heramind_storage::ParsedIntent {
            intent_type,
            target_metrics,
            conditions,
            actions,
