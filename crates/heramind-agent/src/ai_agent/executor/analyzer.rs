...
impl AgentExecutor {
    pub(crate) async fn analyze_situation_with_intent(
        ...
        agent: &AiAgent,
        data: &[DataCollected],
        parsed_intent: Option<&heramind_storage::ParsedIntent>,
        execution_id: &str,
        ...
        match self.get_llm_runtime_for_agent(agent).await {
            ...
            Ok(None) => {
                if agent.llm_backend_id.is_some() {
                    ...
                    return Err(HeraMindError::Validation(format!(
    ...
    pub(crate) async fn analyze_with_llm(
        ...
        agent: &AiAgent,
        data: &[DataCollected],
        _parsed_intent: Option<&heramind_storage::ParsedIntent>,
        execution_id: &str,
        ...
        if let Some(ref bus) = self.event_bus {
            ...
            for decision in &decisions {
                let _ = bus
                    .publish(HeraMindEvent::AgentDecision {
                        agent_id: agent.id.clone(),
                        execution_id: execution_id.to_string(),
