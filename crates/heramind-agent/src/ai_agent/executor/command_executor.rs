...
fn record_notification(
    notifications: &mut Vec<heramind_storage::NotificationSent>,
    channel: &str,
    recipient: &str,
    message: String,
    success: bool,
) {
    notifications.push(heramind_storage::NotificationSent {
...
impl AgentExecutor {
    pub(crate) async fn execute_single_command(
        ...
        decision: &Decision,
    ) -> Option<heramind_storage::ActionExecuted> {
        ...
        Some(heramind_storage::ActionExecuted {
    ...
    async fn handle_query_decision(
        ...
        agent: &AiAgent,
        decision: &Decision,
        actions: &mut Vec<heramind_storage::ActionExecuted>,
    ) {
        ...
        actions.push(heramind_storage::ActionExecuted {
    ...
    pub(crate) async fn execute_decisions(
        ...
        Vec<heramind_storage::ActionExecuted>,
        Vec<heramind_storage::NotificationSent>,
    )> {
            ...
            if agent.execution_mode == heramind_storage::agents::ExecutionMode::Focused {
                agent
                    ...
                    .filter(|r| {
                        matches!(
                            r.resource_type,
                            heramind_storage::ResourceType::Command
                                | heramind_storage::ResourceType::ExtensionTool
                        )
                    })
        ...
        for decision in decisions {
            ...
            if !allowed_command_ids.is_empty() && decision.decision_type == "command" {
                ...
                if !is_allowed {
                    ...
                    actions_executed.push(heramind_storage::ActionExecuted {
    ...
    pub(crate) async fn send_alert_for_decision(
        &self,
        agent: &AiAgent,
        decision: &heramind_storage::Decision,
        notifications_sent: &mut Vec<heramind_storage::NotificationSent>,
    ) {
        ...
        if let Some(ref message_manager) = self.message_manager {
            use heramind_messages::{Message, MessageSeverity};
            ...
            if let Some(ref bus) = self.event_bus {
                let _ = bus
                    .publish(HeraMindEvent::MessageCreated {
    ...
    pub(crate) async fn maybe_generate_report(
        ...
        if let Some(ref intent) = agent.parsed_intent {
            if matches!(
                intent.intent_type,
                heramind_storage::IntentType::ReportGeneration
            ) {
                ...
                return Ok(Some(GeneratedReport {
                    ...
                    data_summary: data
                        .iter()
                        .map(|d| heramind_storage::DataSummary {
