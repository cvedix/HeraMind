...
use heramind_storage::{AiAgent, DataCollected, Decision, DecisionProcess, ReasoningStep};
...
/// tool, or try to call a heramind CLI *domain* (`device`, `dashboard`, ...) as if
    ...
    if matches!(
        ...
        return Some(format!(
            " There is NO `{}` tool. To send notifications or alerts, use the `shell` tool with: \
             `heramind message send --title \"<title>\" --body \"<body>\" --severity \
             <info|warning|error|critical>`.",
            tool_name
        ));
    }
    // 2. heramind CLI domains used as tool names — redirect to shell.
                        ...
                        hallucinated_tool_hint(&result.name).unwrap_or_else(|| {
                            ...
                            format!(
                                " Tool '{}' does not exist. Available tools: {}. \
                                 Use `shell` for any heramind CLI command.",
...
pub(crate) fn build_tool_result(
    ...
    let execution_result = heramind_storage::ExecutionResult {
        actions_executed,
        report: None,
...
mod tests {
    ...
    fn test_hallucinated_tool_hint_redirects_message_aliases() {
        ...
        for name in &[
            ...
            assert!(
                hint.unwrap().contains("heramind message send"),
    ...
    fn test_hallucinated_tool_hint_redirects_cli_domains() {
        ...
        for name in &["device", "dashboard", "rule", "agent", "widget", "system"] {
            ...
            assert!(
                h.contains("heramind"),
                "must reference the heramind CLI: {}",
                h
            );
        }
    }
