...
impl AgentExecutor {
    ...
    fn create_cloud_runtime(
        config: CloudConfig,
        capabilities: &heramind_storage::BackendCapabilities,
    ) -> Result<Arc<dyn LlmRuntime + Send + Sync>, heramind_core::LlmError> {
    ...
    pub async fn get_llm_runtime_for_agent(
        &self,
        agent: &AiAgent,
    ) -> Result<Option<Arc<dyn LlmRuntime + Send + Sync>>, HeraMindError> {
            ...
            if let Some(ref store) = self.llm_backend_store {
                if let Ok(Some(backend)) = store.load_instance(backend_id) {
                        ...
                        .backend_type
                    {
                        ...
                        LlmBackendType::LlamaCpp => {
                            ...
                            crate::llm_backends::backends::llamacpp::LlamaCppRuntime::new(config)
                                .map(|rt| {
                                    ...
                                    std::sync::Arc::new(rt)
                                        as std::sync::Arc<
                                            dyn heramind_core::llm::backend::LlmRuntime
                                                + Send
                                                + Sync,
                                        >
                                })
                        ...
                        LlmBackendType::LlamaCpp => {
                            Err(heramind_core::llm::backend::LlmError::BackendUnavailable(
