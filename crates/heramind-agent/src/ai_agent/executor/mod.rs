...
use crate::error::{HeraMindError, Result as AgentResult};
...
pub(crate) struct ToolLoopOutput {
    ...
    pub(crate) last_llm_error: Option<heramind_core::llm::backend::LlmError>,
}
...
pub(crate) fn resolve_role(agent: &heramind_storage::AiAgent, default: &str) -> String {
    agent
        .system_prompt
        .as_deref()
        .unwrap_or(default)
...
fn build_parameters_schema(
    parameters: &[heramind_core::extension::ParameterDefinition],
) -> serde_json::Value {
    use heramind_core::extension::MetricDataType;
    use std::collections::HashMap;
...
pub struct AgentExecutorConfig {
    ...
    pub time_series_storage: Option<Arc<heramind_storage::TimeSeriesStore>>,
    ...
    pub llm_runtime: Option<Arc<dyn heramind_core::llm::backend::LlmRuntime + Send + Sync>>,
    ...
    pub extension_registry: Option<Arc<heramind_core::extension::registry::ExtensionRegistry>>,
...
pub struct AgentExecutor {
    ...
    /// Time series storage for data collection
    pub(crate) time_series_storage: Option<Arc<heramind_storage::TimeSeriesStore>>,
    /// Device service for command execution
    ...
    /// LLM runtime (default)
    pub(crate) llm_runtime: Option<Arc<dyn heramind_core::llm::backend::LlmRuntime + Send + Sync>>,
    /// LLM backend store for per-agent backend lookup
...
impl AgentExecutor {
    ...
    async fn publish_event(&self, event: HeraMindEvent) {
    ...
    pub async fn set_llm_runtime(
        &mut self,
        llm: Arc<dyn heramind_core::llm::backend::LlmRuntime + Send + Sync>,
    ) {
    ...
    fn filter_tools(
        ...
        Vec<heramind_core::llm::backend::ToolDefinition>,
        std::collections::HashMap<String, String>,
    ) {
        use heramind_core::llm::backend::sanitize_tool_name;
    ...
    async fn execute_with_tools(
    ...
    ) -> AgentResult<(DecisionProcess, heramind_storage::ExecutionResult)> {
        let registry = self
            .tool_registry
            .read()
            .clone()
            .ok_or_else(|| HeraMindError::Tool("Tool registry not available".to_string()))?;
        ...
        let tool_config = match agent.execution_mode {
            heramind_storage::agents::ExecutionMode::Free => ToolLoopConfig::free(),
            heramind_storage::agents::ExecutionMode::Focused => ToolLoopConfig::focused_plus(agent),
        };
        ...
        if llm_generation_failed || (no_tools_executed && has_malformed_output) {
            ...
            return Err(HeraMindError::Llm(msg));
        }
    ...
    pub async fn execute_extension_command(
        ...
        registry
            ...
            .await
            .map_err(|e| HeraMindError::Tool(e.to_string()))
    }
    ...
    async fn send_progress(
        ...
        self.publish_event(heramind_core::HeraMindEvent::AgentProgress {
    ...
    async fn send_thinking(
        ...
        self.publish_event(heramind_core::HeraMindEvent::AgentThinking {
            agent_id: agent_id.to_string(),
            execution_id: execution_id.to_string(),
            step_number,
    ...
    pub async fn execute_agent(
        ...
        self.store
            .update_agent_status(&agent_id, heramind_storage::AgentStatus::Executing, None)
            .await
            .map_err(|e| HeraMindError::Storage(format!("Failed to update status: {}", e)))?;
        ...
        self.publish_event(HeraMindEvent::AgentExecutionStarted {
            agent_id: agent_id.clone(),
        ...
        let execution_result: AgentResult<(DecisionProcess, StorageExecutionResult)> =
            ...
            .await
            {
                ...
                Ok(Err(panic_payload)) => {
                    ...
                    Err(HeraMindError::Llm(format!("Execution panic: {}", msg)))
                }
                Err(_) => {
                    tracing::error!(
                        agent_id = %agent_id,
        ...
        self.store
            ...
            .await
            .map_err(|e| HeraMindError::Storage(format!("Failed to save execution: {}", e)))?;
        ...
        let new_status = if record.status == ExecutionStatus::Completed {
            ...
            heramind_storage::AgentStatus::Error
        };
        ...
        self.publish_event(HeraMindEvent::AgentExecutionCompleted {
            agent_id: agent_id.clone(),
            execution_id: execution_id.clone(),
    ...
    pub async fn execute_agents_parallel(
        ...
        if records.is_empty() && !errors.is_empty() {
            return Err(HeraMindError::Storage(format!(
    ...
    async fn execute_internal(
        ...
        per_exec_knowledge_files: Option<
            Arc<tokio::sync::RwLock<Vec<heramind_storage::KnowledgeFileRef>>>,
        >,
        ...
        match analysis {
            ...
            AnalysisResult::Free {
                ...
                self.store
                    ...
                    .await
                    .map_err(|e| {
                        HeraMindError::Storage(format!("Failed to update memory: {}", e))
                    })?;
