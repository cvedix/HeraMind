...
Custom widgets are React components for HeraMind dashboards. No build tools — just `manifest.json` + `bundle.js`.

## CRITICAL: Widget Development Workflow

```bash
# Step 1: Scaffold
heramind widget create "My Widget" --widget-type chart
...
heramind widget install data/frontend-components/<widget-id>
...
HeraMind provides `window.React`, `window.jsxRuntime.jsx`, and `window.jsxRuntime.jsxs`.
