...
```bash
heramind push list                                # List all push targets
heramind push get <ID>                            # Get target details
heramind push stats                               # Overall push statistics
heramind push logs <ID> [--limit 20]              # Delivery history for a target
```

### Create Push Target

```bash
heramind push create --name <NAME> --type <TYPE> --config '<JSON>' [--schedule <SCHEDULE>] [--sources <PATTERNS>]
```
...
```bash
heramind push enable <ID>                         # Start a stopped target (unified form)
heramind push disable <ID>                        # Stop a running target (unified form)
heramind push start <ID>                          # Legacy alias for enable (hidden, backward compat)
heramind push stop <ID>                           # Legacy alias for disable (hidden, backward compat)
heramind push test <ID>                           # Test delivery (sends sample data)
heramind push update <ID> [--name <N>] [--config '<JSON>'] [--enabled true|false]
heramind push delete <ID>                         # Delete a target
```
...
```bash
heramind push create --name my-webhook --type webhook \
...
```bash
heramind push create --name my-mqtt --type mqtt \
  --config '{"broker": "tcp://broker.example.com:1883", "topic": "heramind/data"}' \
  --schedule event
```
...
heramind push create --name sensor-webhook --type webhook \
  --config '{"url": "https://myapp.example.com/api/heramind-data", "headers": {"Authorization": "Bearer my-token"}}' \
  --sources "device:sensor-001:temperature"

# Step 2: Start the target
heramind push enable <ID>

# Step 3: Test delivery
heramind push test <ID>

# Step 4: Check delivery logs
heramind push logs <ID>
```
...
```bash
# Create MQTT target
heramind push create --name cloud-mqtt --type mqtt \
  --config '{"broker": "tcp://mqtt.cloud.example.com:1883", "topic": "heramind/telemetry", "username": "user", "password": "pass", "qos": 1}' \
  --schedule event
...
```bash
# Update config
heramind push update <ID> --config '{"url": "https://new-endpoint.example.com/data"}'

# Rename
heramind push update <ID> --name "renamed-target"

# Disable without deleting
heramind push update <ID> --enabled false
```
