...
HeraMind supports multiple ways to connect IoT devices. Before helping users, **always check current infrastructure**:
...
MQTT is the primary device connection protocol. HeraMind includes an **embedded MQTT broker** — no external broker needed.

**Broker Info** (from `heramind system info`):
...
> **IMPORTANT**: Always run `heramind system info` first to get the actual broker URL, TLS, auth status, and credentials. The values above are defaults and may differ on the user's system.

**How devices connect:**

1. Device connects to MQTT broker at `<SERVER_IP>:1883`
2. Device publishes data to **any topic**
3. HeraMind auto-discovers the device and creates a draft entry
...
When a device publishes to any MQTT topic, HeraMind:
...
**Webhook URL** (from `heramind system info`):
```
...
**Steps:**
1. Create a device first: `heramind device create --name <NAME> --device-type <TYPE> --adapter-type <ADAPTER>`
...
heramind device types list

# Step 2: Create device with specific type and adapter
heramind device create --name <NAME> --device-type <TYPE> --adapter-type <ADAPTER>
...
heramind device types create --name 'My Sensor' --metrics '[{"name":"temperature","display_name":"Temperature","data_type":"Float","unit":"°C"}]'

# Step 3: Verify device was created
heramind device get <DEVICE_ID>
```
...
const char* mqtt_server = "192.168.1.100";  // from heramind system info
...
heramind device drafts get <DRAFT_ID>

# Approve and register the device
heramind device drafts approve <DRAFT_ID> --name "ESP32 Sensor" --type temp_sensor

# Or reject if unrecognized
heramind device drafts reject <DRAFT_ID>
```

**Auto-discovery configuration:**
```bash
# View current settings
heramind device drafts config

# Enable auto-approve (skip manual review)
heramind device drafts config --auto-approve true

# Disable auto-discovery
heramind device drafts config --enabled false
```
...
Use the same connection info from `heramind system info`. See the Python example in Scenario A above for TLS + auth configuration.
...
```bash
heramind device create --name "Weather Station" --device-type weather-station --adapter-type webhook
```

2. Get the webhook URL for the device:
```bash
heramind device webhook-url <DEVICE_ID>
```
...
1. HeraMind supports connecting to external brokers.
2. The external broker can be configured via the Web UI (Settings > MQTT).
3. Once configured, HeraMind subscribes to the external broker and auto-discovers devices.
...
Run `heramind system info` and check `mqtt.broker_address` and `mqtt.broker_url`. The URL includes the protocol scheme (`mqtt://` or `mqtts://`).

### "Do I need to install an MQTT broker?"
No. HeraMind includes an embedded MQTT broker. Devices can connect directly.

### "Does MQTT require authentication?"
Check `heramind system info` → `mqtt.auth_enabled`. If true, use the credentials from `mqtt.credentials` array. If false, anonymous connections are accepted.

### "Is TLS enabled?"
Check `heramind system info` → `mqtt.tls_enabled`. If true, use `mqtts://` scheme and ensure the device trusts the CA cert. If `tls_ca_available` is true, the CA cert can be downloaded from the web UI (Settings → MQTT Broker).

### "How do I know if my device is connected?"
```bash
heramind system info    # Check mqtt.connected and mqtt.devices_connected
heramind device list    # See all registered devices
heramind device get <ID>  # Check latest data from a device
```

### "My device sends data but it doesn't appear"
1. Check MQTT broker is connected: `heramind system info` → `mqtt.connected`
...
Yes! Any topic works. HeraMind auto-discovers data from any topic.
...
These typically require a **gateway** that translates the protocol to MQTT or HTTP. The gateway sends data to HeraMind via MQTT or webhook.
...
| Item | Value |
|------|-------|
| MQTT Broker | `<SERVER_IP>:1883` (embedded, check `heramind system info` for actual) |
...
| Auto-Discovery | Enabled (`heramind/discovery/#`) |
| External Brokers | `heramind connector list` to see all |
| Webhook URL | `POST http://<SERVER_IP>:9375/api/devices/{device_id}/webhook` |
| API Base | `http://<SERVER_IP>:9375/api` |
| Check System | `heramind system info` |

## Device Management Commands

| Command | Description |
|---------|-------------|
| `heramind device list` | List all devices |
| `heramind device get <ID>` | Get device details |
| `heramind device create --name <NAME> [--device-type <T>] [--adapter-type <A>]` | Create device |
| `heramind device update <ID> [--name <N>] [--config '<JSON>']` | Update device |
...
| `heramind device webhook-url <ID>` | Get webhook push URL |
| `heramind device drafts list` | List pending device drafts |
| `heramind device drafts get <ID>` | View draft details and sample data |
| `heramind device drafts approve <ID> --name <N> --type <T>` | Approve and register draft |
| `heramind device drafts reject <ID>` | Reject and discard draft |
| `heramind device drafts config [--enabled] [--auto-approve] [--max-samples]` | View/configure auto-discovery |

## Connector Management

| Command | Description |
|---------|-------------|
| `heramind connector list` | List all MQTT connectors |
| `heramind connector create --name <N> --host <H> [--port <P>]` | Add external connector |
| `heramind connector get <ID>` | Get connector details |
| `heramind connector test <ID>` | Test connector connection |
| `heramind connector subscriptions` | List active subscriptions |
...
```bash
heramind device create --name 'Weather Station' --adapter-type webhook
...
- **"MQTT broker not connected"**: The server may not be running. Check with `heramind system info`. If `mqtt.connected` is false, restart the server.
- **"Device not appearing after sending data"**: Verify the device is sending to the correct broker IP/port. Check network connectivity. Run `heramind device list` to see registered devices.
...
- **"Auth failed / Not authorized"**: Auth is enabled on the broker. Run `heramind system info` to get valid credentials from `mqtt.credentials`, then configure the device with username/password.
...
- **"Webhook returns 404"**: The device must be created first via `heramind device create` before sending webhook data. Use the exact device ID from the create response.
- **"Data not updating"**: Run `heramind device get <ID>` to check latest readings. If stale, verify the device is still publishing. Check `heramind system info` for MQTT connection status.
- **"Device sent data but not in device list"**: New devices appear as drafts. Run `heramind device drafts list` to find pending devices, then `heramind device drafts approve <ID>` to register.
- **"Too many unknown devices appearing"**: Adjust auto-discovery settings with `heramind device drafts config --max-samples 5` or disable with `--enabled false`.
