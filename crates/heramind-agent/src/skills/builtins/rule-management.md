...
- [ ] Ran `heramind device list` (or `heramind extension get <id>` for extension metrics)
...
- [ ] If `metric_fields` is empty OR device offline OR >50 devices: ran `heramind device get <id>` per target
...
Before running `heramind rule create`, tick every box:
...
- [ ] If action is `notify`: at least one message channel exists — run `heramind message channel-list`. If empty, load **message-management** skill first.
- [ ] If action is `execute`: target device exists in `heramind device list` and has the command listed under `command_fields`
...
heramind rule test <ID> --input '{"<metric>": <value_above_threshold>}'
...
1. **Rule enabled?** `heramind rule get <ID>` → check `enabled: true`. If false: `heramind rule enable <ID>`.
2. **Device actually online?** `heramind device list` → device status. If offline, rule has no data to evaluate.
3. **Data fresh?** `heramind device get <ID>` → last metric value. If stale (older than `offline_timeout`), no recent evaluations.
4. **Condition match at least once?** `heramind rule history <ID>` → look for `evaluated: true, matched: false`. If `matched: true` but no notification → channel problem (jump to message-management).
...
heramind rule test <ID> --input '<JSON>'   # inject synthetic metric
heramind rule history <ID>                 # evaluation log
```
