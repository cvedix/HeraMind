...
heramind extension get <EXTENSION_ID>
...
```bash
heramind transform create --name '<name>' --scope <scope> --code '<JS>'
```
...
heramind transform test-code --code 'return (input - 32) * 5 / 9' --input '{"value": 212}'
```
...
heramind transform get <ID>              # Get transform details
heramind transform metrics               # List all virtual metrics produced by transforms
heramind transform data-sources          # List available data sources for transforms
```

### Update & Delete

```bash
heramind transform update <ID> --code '<NEW_JS>'    # Update code
heramind transform enable <ID>                       # Enable (unified form)
heramind transform disable <ID>                      # Disable (unified form)
heramind transform delete <ID>                       # Delete transform
```
...
heramind transform create --name 'Fahrenheit to Celsius' \
  --scope global \
...
```bash
heramind transform test-code --code 'if (input > 80) return "good"; if (input > 20) return "ok"; return "low"' --input '{"value": 15}'

heramind transform create --name 'Battery Health' \
  --scope global \
...
```bash
heramind transform create --name 'Sensor-001 Calibration' \
...
```bash
heramind transform create --name 'Percentage Format' \
  --scope global \
...
```bash
heramind transform create --name 'Status Text' \
  --scope global \
...
heramind transform test-code --code 'const w = extensions.invoke("weather", "current", {city: "Shanghai"}); return {outdoor: w.temperature, indoor: input}' --input '{"value": 22}'

heramind transform create --name 'Indoor vs Outdoor' \
  --scope global \
...
heramind dashboard add-components <DASHBOARD_ID> --components '[{
  "id": "t1",
  "type": "value-card",
...
| `extensions.invoke is not defined` | Extension not installed | Check `heramind extension list` and verify extension ID |
