...
```bash
heramind agent create --name 'Monitor' --prompt 'Check battery levels'
# → Returns agent ID (e.g., agent-abc123), already active and running

# To pause later:
heramind agent control agent-abc123 paused

# To resume:
heramind agent control agent-abc123 active
```
...
**Important**: `--llm-backend` selects which LLM powers the agent. Run `heramind llm list` first and match backend capabilities to your task (see "Choosing the Right LLM Backend" above).

### Control Agent

```bash
heramind agent control <ID> active    # Start (positional status)
heramind agent control <ID> paused    # Stop
```

### Invoke (One-shot Execution)

```bash
heramind agent invoke <ID> 'Analyze current temperature sensors'    # Positional input
```

### Get Details & Status

```bash
heramind agent get <ID>          # Full config + status
heramind agent list              # All agents
```

### Update Agent

```bash
heramind agent update <ID> --prompt 'New task description'
heramind agent update <ID> --llm-backend qwen3.5:4b
heramind agent update <ID> --name 'Better Name' --description 'Updated'
```

### Monitor Executions

```bash
heramind agent executions <ID> --limit 10      # Execution history
heramind agent latest-execution <ID>           # Most recent execution
heramind agent conversation <ID> --limit 20    # Full message log
heramind agent memory <ID>                     # Execution journal + knowledge files
```

### Send Message

```bash
heramind agent send-message <ID> 'Focus on building A sensors'    # Positional message
heramind agent send-message <ID> 'Directive' --message-type instruction
```
...
heramind agent latest-execution <AGENT_ID>
```
...
# Run whenever needed
heramind agent invoke <AGENT_ID> 'Analyze temperature trends for sensor-001'
```
...
heramind agent get <ID>

# 2. See recent execution results (check status, duration, error)
heramind agent latest-execution <ID>

# 3. If latest execution failed, check full conversation to see what happened
heramind agent conversation <ID> --limit 20

# 4. Check if LLM backend is available
heramind llm list
# If the configured model is not available, update:
heramind agent update <ID> --llm-backend <available_backend>

# 5. Check agent memory (execution journal + knowledge files)
heramind agent memory <ID>

# 6. If agent is stuck in a loop, pause and review
heramind agent control <ID> paused
heramind agent conversation <ID> --limit 50

# 7. After fixing, re-activate
heramind agent control <ID> active
```

### Full Lifecycle

```bash
heramind agent create --name 'Health Check' --prompt 'Check all devices' --every 10m
# ... agent starts immediately, check results ...
heramind agent latest-execution <ID>
heramind agent control <ID> paused    # Stop when done
heramind agent delete <ID>                      # Remove when no longer needed
```
...
|-------|-------|----------|
| "Agent not found" | Wrong ID | Run `heramind agent list` for valid IDs |
...
| Bad LLM responses | Wrong model/backend | Run `heramind llm list` for available backends, update with `agent update <ID> --llm-backend <backend>` |
