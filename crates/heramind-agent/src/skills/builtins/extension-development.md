...
token_budget: 14000
triggers:
  keywords: [extension development, create extension, build extension, extension sdk, heramind extension, 扩展开发, 开发扩展, extension create, extension build, custom extension, heramind_export, Extension trait, capability, FFI, native extension, .nep]
  tool_target:
    - tool: extension
...
HeraMind extensions are Rust programs running in isolated processes. They communicate with the platform via FFI (ABI version 3).
...
HeraMind Server
  └── Extension Runner (isolated process)
        └── Your Extension (.dylib/.so/.dll)
              ...
              ├── exposes FFI via heramind_export!()
...
```bash
heramind device list                    # What devices exist?
heramind device get <DEVICE_ID>      # What metrics do they have?
heramind extension list                 # What extensions are installed?
heramind llm list                       # What LLM backends available?
heramind system info                    # MQTT broker, webhook URL
```

### Step 1: Scaffold

```bash
heramind extension create my-extension --extension-type tool -o ./extensions
```
...
```
file_write(path="extensions/my-extension/src/lib.rs", content="use heramind_extension_sdk::prelude::*;\n...")
file_edit(path="extensions/my-extension/Cargo.toml", old_string='name = "old-name"', new_string='name = "new-name"')
```

If the extension source is outside the data directory, use shell commands or set `HERAMIND_ALLOWED_WRITE_DIRS` env var to include the development directory.

### Step 3: Build & Install

```bash
heramind extension build ./extensions/my-extension
heramind extension validate ./my-extension.nep --verbose
heramind extension install ./my-extension.nep
heramind extension status my-extension     # verify running
heramind extension logs my-extension       # check for errors
```
...
```toml
[package]
name = "heramind-extension-my-extension"
version = "1.0.0"
edition = "2021"

[lib]
name = "heramind_extension_my_extension"
crate-type = ["cdylib", "rlib"]

[dependencies]
heramind-extension-sdk = { path = "../../crates/heramind-extension-sdk" }
...
```rust
use heramind_extension_sdk::ExtensionError;
...
| Command | Description |
|---------|-------------|
| `heramind extension list` | List all installed extensions |
| `heramind extension get <ID>` | Get extension details (alias: `info`) |
| `heramind extension status <ID>` | Get runtime status |
| `heramind extension logs <ID> [--limit <N>]` | View extension logs |
| `heramind extension reload <ID>` | Reload (pick up code changes) |
| `heramind extension config <ID>` | View current config |
| `heramind extension config <ID> --set '<JSON>'` | Update config |
| `heramind extension install <PATH>` | Install from .nep |
| `heramind extension uninstall <ID>` | Uninstall extension |
