...
impl ToolNameMapper {
    ...
    fn register_builtin_mappings(&mut self) {
        ...
        // Route them to the shell tool, which executes `heramind <domain> <action>` commands.
...
/// Convert a CLI domain tool call into a `heramind` CLI command for shell execution.
///
/// `original_tool_name` is what the LLM called (e.g., `"message"`, `"device"`).
/// Returns `Some({"command": "heramind <domain> <action> --flag value ..."})` suitable
...
pub fn build_cli_command(original_tool_name: &str, arguments: &Value) -> Option<Value> {
    ...
    let mut cmd = format!("heramind {} {}", original_tool_name, action);
