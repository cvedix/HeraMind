...
impl CapabilityIndex {
    ...
    pub fn build_data_conventions() -> String {
        r###"
### Data Conventions
- Run `heramind device list` first for the overview — read `summary`, `types[].type`, `types[].metric_fields` (field names per type), and `types[].devices.list` (id/name/status). Filter with `--device-type <type>` (not `--type`) or `--status`. Fetch a device's current values with `device get <id>`, history with `device history <id> --metric <field> --time-range 7d`.
...
- device history: `heramind device history <ID> --metric <field> --time-range <range> [--compress=true]`
...
mod tests {
    ...
    fn data_conventions_mentions_key_rules() {
        ...
        assert!(c.contains("HERAMIND_JSON"));
