...
pub struct PathValidator {
    ...
    data_dir: PathBuf,
    /// Additional allowed directories (from HERAMIND_ALLOWED_WRITE_DIRS).
...
impl PathValidator {
    ...
    /// Load extra allowed directories from the `HERAMIND_ALLOWED_WRITE_DIRS` env var.
    /// Multiple directories separated by `:`.
    fn load_extra_dirs() -> Vec<PathBuf> {
        if let Ok(dirs) = std::env::var("HERAMIND_ALLOWED_WRITE_DIRS") {
            dirs.split(':')
                .filter(|s| !s.is_empty())
                .map(PathBuf::from)
                .filter(|p| p.exists())
