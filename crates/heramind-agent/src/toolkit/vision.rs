...
impl VisionTool {
    ...
    async fn resolve_vlm_candidates(
        ...
        if candidates.is_empty() {
            return Err(ToolError::Execution(
                "No vision model configured. Install a VLM (e.g., qwen2.5-vl, minicpm-v) via Ollama and `heramind llm activate` it.".into(),
            ));
        }
    ...
    async fn analyze(
        ...
        for (id, runtime) in &candidates {
            ...
            let input = LlmInput {
                messages: vec![msg],
                params: heramind_core::llm::GenerationParams {
        ...
        Err(ToolError::Execution(format!(
            "VLM inference failed on all {} candidate backend(s) — {}. Activate a working multimodal backend via `heramind llm activate <id>` (its model must be installed in Ollama).",
            candidates.len(),
            errors.join("; ")
        )))
    }
    ...
    fn parameters(&self) -> Value {
        object_schema(
            serde_json::json!({
                ...
                "model": serde_json::json!({"type": "string", "description": "Optional VLM model name to force (e.g. `minicpm-v4.6`). Use when the auto-picked model says it can't see the image. Find names via `heramind llm list`."})
            }),
