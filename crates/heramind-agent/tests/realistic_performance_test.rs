    ...
    async fn llm_analyze(&self, system_prompt: &str, user_input: &str) -> (String, u128) {
        ...
        let output = self.llm_runtime.generate(input).await.unwrap_or_else(|e| {
            heramind_core::llm::backend::LlmOutput {
                text: format!("Error: {}", e),
                finish_reason: heramind_core::llm::backend::FinishReason::Error,
                usage: None,
                thinking: None,
                tool_calls: None,
            }
    ...
    let parallel_futures: Vec<_> = queries
        ...
        .map(|query| {
            ...
            tokio::spawn(async move {
                let messages = vec![
                    heramind_core::message::Message::new(
                        heramind_core::message::MessageRole::System,
                        heramind_core::message::Content::text(&prompt),
                    ),
                    heramind_core::message::Message::new(
                        heramind_core::message::MessageRole::User,
                        heramind_core::message::Content::text(&q),
                    ),
                ];

                let input = heramind_core::llm::backend::LlmInput {
                    messages,
