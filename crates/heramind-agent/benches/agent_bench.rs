...
use heramind_agent::agent::AgentMessage;
...
fn bench_token_estimation(c: &mut Criterion) {
    ...
    c.bench_function("estimate_tokens_10_messages", |b| {
        b.iter(|| {
            let mut total = 0;
            for msg in &messages {
                total += black_box(heramind_agent::agent::tokenizer::estimate_message_tokens(
                    msg,
                ));
            }
            black_box(total)
...
fn bench_context_compaction(c: &mut Criterion) {
    c.bench_function("compact_10_messages", |b| {
        b.iter_batched(
            ...
            |messages| black_box(heramind_agent::agent::compact_tool_results(&messages, 2)),
...
fn bench_conversation_context(c: &mut Criterion) {
    c.bench_function("context_add_devices", |b| {
        b.iter_batched(
            heramind_agent::agent::ConversationContext::new,
            |mut ctx| {
                for i in 0..10 {
...
fn bench_memory_operations(c: &mut Criterion) {
    c.bench_function("memory_clone_10_messages", |b| {
        b.iter_batched(
            || {
                let mut state = heramind_agent::agent::AgentInternalState::new("test".to_string());
                for i in 0..10 {
