...
use heramind_core::extension::isolated::IsolatedExtensionError;
use heramind_core::extension::stream::{
...
fn test_extension_descriptor_with_capabilities() {
    ...
    let metrics = vec![MetricDescriptor {
        ...
        data_type: heramind_core::extension::system::MetricDataType::Integer,
...
fn test_proxy_module_exports() {
    ...
    use heramind_core::extension::proxy::{create_proxy, create_proxy_with_descriptor};
...
fn test_flow_control_defaults() {
    let fc = heramind_core::extension::stream::FlowControl::default();
...
fn test_client_info_for_proxy() {
    let info = ClientInfo {
        ...
        user_agent: Some("HeraMind-Proxy/1.0".to_string()),
    };
...
impl Extension for MockProxyExtension {
    ...
    ) -> heramind_core::extension::system::Result<serde_json::Value> {
        match command {
