...
fn test_extension_metadata_builder() {
    let meta = ExtensionMetadata::new("heramind.weather.live", "Live Weather Provider", "1.0.0")
        .with_description("Provides real-time weather data")
        .with_author("HeraMind Team");

    assert_eq!(meta.id, "heramind.weather.live");
    ...
    assert_eq!(meta.author, Some("HeraMind Team".to_string()));
}
