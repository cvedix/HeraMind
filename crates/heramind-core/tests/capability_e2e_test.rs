...
use heramind_core::extension::isolated::{IsolatedExtensionManager, IsolatedManagerConfig};
...
fn smoke_extension_path() -> PathBuf {
    let lib_name = if cfg!(target_os = "macos") {
        "libheramind_smoke_extension.dylib"
    } else if cfg!(target_os = "windows") {
        "heramind_smoke_extension.dll"
    } else {
        "libheramind_smoke_extension.so"
    };
