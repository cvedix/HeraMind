...
use heramind_core::extension::context::{
...
use heramind_core::extension::ExtensionRuntime;
use serde_json::json;
...
async fn test_complete_weather_extension_workflow() {
    ...
    registry
        .register("heramind.weather.forecast".to_string(), weather_ext)
        .await
        .unwrap();
    ...
    let metrics = registry
        .get_current_metrics("heramind.weather.forecast")
        .await;
    assert!(!metrics.is_empty());

    // Health check
    let health = registry
        .health_check("heramind.weather.forecast")
        .await
        .unwrap();
    assert!(health);
    ...
    // Unregister
    registry
        .unregister("heramind.weather.forecast")
        .await
        .unwrap();
    assert!(!registry.contains("heramind.weather.forecast").await);
}
...
async fn test_complete_sensor_extension_workflow() {
    ...
    registry
        .register("heramind.sensor.environment".to_string(), sensor_ext)
        .await
        .unwrap();

    // Read all sensors
    let result = registry
        .execute_command("heramind.sensor.environment", "read_all", &json!({}))
        .await
        .unwrap();
    ...
    // Get metrics
    let metrics = registry
        .get_current_metrics("heramind.sensor.environment")
        .await;
...
#[async_trait]
impl heramind_core::extension::context::ExtensionCapabilityProvider for MockCapabilityProvider {
    fn capability_manifest(&self) -> heramind_core::extension::context::CapabilityManifest {
        heramind_core::extension::context::CapabilityManifest {
            capabilities: vec![
    ...
    ) -> std::result::Result<serde_json::Value, heramind_core::extension::context::CapabilityError>
    {
        match capability {
            ...
            _ => Err(heramind_core::extension::context::CapabilityError::NotAvailable(capability)),
        }
    }
}
...
async fn test_high_volume_command_execution() {
    ...
    for _ in 0..100 {
        ...
        let handle: tokio::task::JoinHandle<Result<serde_json::Value>> = tokio::spawn(async move {
            reg.execute_command("heramind.sensor.environment", "read_all", &json!({}))
                .await
        });
        handles.push(handle);
    }
