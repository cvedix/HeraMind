...
/// Strip a `__HERAMIND_TOKEN_PROMPT:N__` marker from a stream chunk and
...
fn strip_token_marker(content: &str) -> (String, Option<u32>) {
    const MARKER: &str = "__HERAMIND_TOKEN_PROMPT:";
...
pub trait LlmRuntime: Send + Sync {
    ...
    /// - token-usage markers (`\n__HERAMIND_TOKEN_PROMPT:N__`) are filtered out
    ...
    /// - `prompt_tokens`: real, extracted from `__HERAMIND_TOKEN_PROMPT:N__`
    ...
    async fn generate_to_completion(
        ...
        // prompt_tokens is extracted in-band from `__HERAMIND_TOKEN_PROMPT:N__`
        // markers emitted by streaming backends (Ollama/OpenAI/llama.cpp).
...
mod tests {
            ...
            let chunks: Vec<StreamChunk> = self
                ...
                .map(|c| match c {
                    ...
                    MockChunk::TokenMarker(n) => {
                        Ok((format!("\n__HERAMIND_TOKEN_PROMPT:{}__", n), false))
                    }
    ...
    async fn generate_to_completion_captures_prompt_token_marker() {
        // Streaming backends inject `\n__HERAMIND_TOKEN_PROMPT:N__` markers to
    ...
    async fn generate_to_completion_handles_embedded_token_marker() {
        ...
        let runtime = MockStreamRuntime {
            chunks: vec![MockChunk::Content(
                "before __HERAMIND_TOKEN_PROMPT:99__ after".into(),
            )],
        };
        let out = runtime
            .generate_to_completion(LlmInput::new("hi"))
        ...
        assert!(!out.text.contains("HERAMIND_TOKEN"));
        let usage = out.usage.expect("usage must be populated");
