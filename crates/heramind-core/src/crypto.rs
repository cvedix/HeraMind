...
//!
//! Lives in heramind-core so every crate (api, storage, …) can share the same
...
const ENCRYPTION_KEY_ENV: &str = "HERAMIND_ENCRYPTION_KEY";
...
impl CryptoService {
    ...
    ///
    /// Priority:
    /// 1. `HERAMIND_ENCRYPTION_KEY` environment variable
    ...
    fn derive_key(input: &[u8]) -> [u8; 32] {
        ...
        let salt = b"HeraMind-API-Key-Salt-2024";
        let mut key = [0u8; 32];
    ...
    pub fn hash_api_key(&self, api_key: &str) -> String {
        ...
        hasher.update(b"HeraMind-API-Key-v1");
