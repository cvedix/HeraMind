// Integration Tests for HeraMind
//
...
async fn test_user_to_llm_to_device_flow() {
    ...
    let mut user_rx = bus
        .filter()
        .custom(|e| matches!(e, HeraMindEvent::UserMessage { .. }));

    // Subscribe to tool execution events
    let _tool_rx = bus
        .filter()
        .custom(|e| matches!(e, HeraMindEvent::ToolExecutionStart { .. }));

    // Publish user message
    bus.publish(HeraMindEvent::UserMessage {
...
async fn test_llm_periodic_review_flow() {
    use heramind_core::event::{HeraMindEvent, ProposedAction};
    use heramind_core::eventbus::EventBus;
