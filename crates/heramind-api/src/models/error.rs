...
impl From<heramind_agent::HeraMindError> for ErrorResponse {
    fn from(e: heramind_agent::HeraMindError) -> Self {
        Self::internal(format!("Agent error: {}", e))
    }
}

impl From<heramind_devices::DeviceError> for ErrorResponse {
    fn from(e: heramind_devices::DeviceError) -> Self {
        match e {
            heramind_devices::DeviceError::NotFound(id) => {
                Self::not_found(format!("Device '{}'", id)).with_hint(
                    "Run 'heramind device list' to see available devices and their IDs.",
                )
            }
            heramind_devices::DeviceError::InvalidParameter(msg) => {
                Self::bad_request(msg.to_string()).with_hint(
                    "Check required fields: --name (string), --type (device type ID), --adapter (mqtt|webhook|http-poll|ble|modbus-tcp|serial).\n\
                     Run 'heramind device types list' to see valid device types.",
                )
            }
            _ => Self::internal(e.to_string()),
        }
...
impl From<heramind_rules::RuleError> for ErrorResponse {
    fn from(e: heramind_rules::RuleError) -> Self {
        let msg = format!("{}", e);
        let hint = match &msg {
