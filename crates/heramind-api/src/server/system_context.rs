...
use heramind_core::llm::backend::{GenerationParams, LlmInput, LlmRuntime};
use heramind_storage::{AgentFilter, AgentStore, MarkdownMemoryStore, SessionStore};
            ...
            .await
        {
            Ok(agents) => {
                let active: Vec<_> = agents
                    .iter()
                    .filter(|a| a.status != heramind_storage::AgentStatus::Stopped)
                    .collect();
                if !active.is_empty() {
