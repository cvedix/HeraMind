...
pub struct CredentialCache {
    ...
    /// System password for `__heramind_internal__` (plaintext, never logged).
...
impl CredentialCache {
    ...
    pub fn load_from_store(store: &heramind_storage::SettingsStore) -> Result<Self, String> {
...
use heramind_agent::SessionManager;
use heramind_core::{extension::ExtensionRegistry, EventBus};
use heramind_devices::adapter::AdapterResult;
use heramind_devices::{DeviceRegistry, DeviceService, TimeSeriesStorage};
use heramind_rules::{
...
use heramind_storage::dashboards::DashboardStore;
use heramind_storage::frontend_components::FrontendComponentStore;
use heramind_storage::instances::InstanceStore;
use heramind_storage::llm_backends::LlmBackendStore;

use crate::automation::{
    store::SharedAutomationStore, transform::TransformEngine, AutoOnboardManager,
};

use heramind_data_push::PushManager;
use heramind_messages::MessageManager;
...
#[cfg(feature = "embedded-broker")]
use heramind_devices::{EmbeddedBroker, TopicResolverFn};
...
impl ServerState {
    ...
    pub async fn restart_embedded_broker(&self) -> Result<(), String> {
        ...
        let credential_validator: CredentialValidator =
            std::sync::Arc::new(move |username: &str, password: &str| {
                ...
                if username == "__heramind_internal__" {
                        ...
                        .and_then(|s| s.get_system_mqtt_credential().ok().flatten());
                    {
                        ...
                        let rollback_mqtt_config = MqttAdapterConfig {
                            ...
                            mqtt: heramind_devices::mqtt::MqttConfig {
                                ...
                                port: old_cfg.port,
                                client_id: Some("heramind-internal".to_string()),
                                username: adapter_pass
                                    .as_ref()
                                    .map(|_| "__heramind_internal__".to_string()),
        ...
        *self.devices.embedded_broker.write().unwrap() = Some(Arc::new(broker));
        ...
        let (adapter_username, adapter_password) = {
            if let Ok(store) = open_settings_store() {
                match store.get_system_mqtt_credential() {
                    Ok(Some(pass)) => (Some("__heramind_internal__".to_string()), Some(pass)),
                    _ => {
                        tracing::warn!(
        ...
        if let Some(mqtt) = mqtt_adapter
            .as_any()
            .downcast_ref::<heramind_devices::adapters::mqtt::MqttAdapter>()
        {
    ...
    pub fn agent_store(&self) -> Arc<heramind_storage::AgentStore> {
...
impl ServerState {
    ...
    pub async fn new() -> Self {
        ...
        let agent_store_h = tokio::task::spawn_blocking(
            || match heramind_storage::AgentStore::open("data/agents.redb") {
                ...
                Err(e) => {
                    ...
                    heramind_storage::AgentStore::memory().unwrap_or_else(|e| {
        ...
        // Use HERAMIND_DATA_DIR if set, otherwise use relative path
        ...
        // Set up extension command router so DeviceService can route commands to extensions
        {
            ...
            let router: heramind_devices::ExtensionCommandRouterFn = Arc::new(
            ...
            Arc::new(tokio::sync::RwLock::new(None));
        {
            ...
            use heramind_core::extension::CapabilityServices;

            let services = CapabilityServices::new()
                .with_service(
                    heramind_core::extension::keys::DEVICE_SERVICE,
                    devices.service.clone(),
                )
                .with_service(
                    heramind_core::extension::keys::TELEMETRY_STORAGE,
                    devices.telemetry.clone(),
                )
                .with_service(
                    heramind_core::extension::keys::RULE_ENGINE,
                    rule_engine.clone(),
                )
                .with_service(
                    heramind_core::extension::keys::EXTENSION_REGISTRY,
                    extensions.registry.clone(),
                )
                .with_service(
                    heramind_core::extension::keys::EVENT_BUS,
                    event_bus
                        .clone()
    ...
    pub async fn new_for_testing() -> Self {
        ...
        let agent_store = heramind_storage::AgentStore::memory().unwrap();
        let system_memory_store = Arc::new(heramind_storage::MarkdownMemoryStore::new(
            std::env::temp_dir().join("heramind-test-memory"),
        ));
        ...
        let frontend_component_store = FrontendComponentStore::open(
            std::env::temp_dir().join(format!("heramind-test-fc-{}", uuid::Uuid::new_v4())),
        )
    ...
    pub async fn start_embedded_broker(&self) {
        ...
        // MQTT client_id to HeraMind device_id by observing publish topics.
        if let Some(resolver) = self.build_topic_resolver() {
            broker.set_topic_resolver(resolver);
        }
    ...
    pub async fn init_device_adapters(&self) {
        ...
        {
            use heramind_devices::adapters::{create_adapter, webhook::WebhookAdapterConfig};
            ...
            match create_adapter("webhook", &webhook_config_value, event_bus) {
                Ok(adapter) => {
                    ...
                    if let Some(whk) = adapter
                        .as_any()
                        .downcast_ref::<heramind_devices::adapters::webhook::WebhookAdapter>(
                    ) {
    ...
    /// Domain tools (device, agent, rule, message, etc.) are handled via `heramind` CLI
    ...
    pub async fn init_tools(&self) {
        ...
        let mut registry = ToolRegistryBuilder::new()
            ...
            // Shell tool — CLI-first: domain operations via `heramind` commands
            .with_shell_tool(Some(heramind_agent::toolkit::ShellConfig {
                enabled: true,
                timeout_secs: 30,
        ...
        {
            let tool = heramind_agent::toolkit::skill_tool::SkillTool::with_data_dir(
        ...
        registry.register(Arc::new(heramind_agent::toolkit::WebFetchTool::new()));
        // File write tool — creates/overwrites files in data/
        registry.register(Arc::new(heramind_agent::toolkit::FileWriteTool::new(
            self.data_dir.clone(),
        )));
        // File edit tool — precise string replacement in files
        registry.register(Arc::new(heramind_agent::toolkit::FileEditTool::new(
            self.data_dir.clone(),
        )));

        // Vision tool — auto-detect VLM backend and register if available
        if let Ok(llm_manager) = heramind_agent::llm_backends::get_instance_manager() {
            let has_vlm = llm_manager
                .list_instances()
                .iter()
                .any(|inst| inst.capabilities.supports_multimodal);
            if has_vlm {
                registry.register(Arc::new(heramind_agent::toolkit::VisionTool::new(
                    heramind_agent::toolkit::VisionConfig::default(),
                    llm_manager,
                )));
                tracing::info!(
                    category = "ai",
        ...
        registry.register(Arc::new(heramind_agent::toolkit::ImageEditTool::new(
            self.data_dir.clone(),
        )));
        ...
        // Uses per-execution handle swapping for concurrency safety
        {
            ...
            let memory_tool = heramind_agent::toolkit::MemoryTool::with_session_handle(
    ...
    pub async fn init_extension_event_subscription(&self) {
        ...
        use heramind_core::extension::ExtensionEventSubscriptionService;
    ...
    pub async fn init_rule_engine_events(&self) {
            ...
            while let Some((event, _metadata)) = rx.recv().await {
                ...
                } = event
                {
                    ...
                    let rule_value = match &value {
                        MetricValue::Float(v) => Some(heramind_rules::RuleValue::Number(*v)),
                    ...
                    if let Some(rv) = rule_value {
                        ...
                        {
                            match &rv {
                                heramind_rules::RuleValue::Number(num_value) => {
                                ...
                                heramind_rules::RuleValue::Text(text_value) => {
                                    // Store with original metric key
                                    value_provider
                                        .update_string_value(
                        ...
                        let data_source =
                            heramind_core::datasource::DataSourceId::device(&device_id, &metric);
                        ...
                        for prefix in &common_prefixes {
                            if let Some(stripped_metric) = metric.strip_prefix(prefix) {
                                let stripped_source =
                                    heramind_core::datasource::DataSourceId::device(
                                        &device_id,
                                        stripped_metric,
                                    );
                                rule_engine_for_update
        ...
        tokio::spawn(async move {
            ...
            while let Some((event, _)) = ext_rx.recv().await {
                if let HeraMindEvent::ExtensionOutput {
                    extension_id,
                    output_name,
                    value,
                    ..
                } = event
                {
                    ...
                    let rv = match &value {
                        MetricValue::Float(v) => heramind_rules::RuleValue::Number(*v),
                        MetricValue::Integer(v) => heramind_rules::RuleValue::Number(*v as f64),
                        MetricValue::Boolean(v) => {
                            heramind_rules::RuleValue::Number(if *v { 1.0 } else { 0.0 })
                        }
                        MetricValue::String(s) => heramind_rules::RuleValue::Text(s.clone()),
                        MetricValue::Json(v) => heramind_rules::RuleValue::Text(v.to_string()),
                    };
                    ...
                    // Notify rule engine
                    let data_source = heramind_core::datasource::DataSourceId::extension(
                        &extension_id,
                        metric_name,
                    );
        ...
        // Wire agent trigger callback for rule TriggerAgent actions
        {
            ...
            let callback: heramind_rules::AgentTriggerCallback = Arc::new(
    ...
    pub async fn init_auto_onboarding_events(&self) {
        ...
        let auto_onboard_manager = {
            ...
            if let Some(mgr) = mgr_guard.as_ref() {
                ...
                if let Some(mgr) = mgr_guard.as_ref() {
                    ...
                    use heramind_agent::llm_backends::backends::{OllamaConfig, OllamaRuntime};
                    use heramind_core::llm::backend::LlmRuntime;

                    let config = OllamaConfig::new(heramind_core::config::models::OLLAMA_DEFAULT)
                    ...
                    let llm = match OllamaRuntime::new(config) {
                        ...
                        Err(_) => {
                            ...
                            impl LlmRuntime for DummyRuntime {
                                fn backend_id(&self) -> heramind_core::llm::backend::BackendId {
                                    heramind_core::llm::backend::BackendId::new("dummy")
                                }
                                ...
                                fn capabilities(
                                    &self,
                                ) -> heramind_core::llm::backend::BackendCapabilities
                                {
                                    heramind_core::llm::backend::BackendCapabilities::default()
                                }
                                async fn generate(
                                    &self,
                                    _input: heramind_core::llm::backend::LlmInput,
                                ) -> Result<
                                    heramind_core::llm::backend::LlmOutput,
                                    heramind_core::llm::backend::LlmError,
                                > {
                                    Ok(heramind_core::llm::backend::LlmOutput {
                                        text: String::new(),
                                        thinking: None,
                                        finish_reason:
                                            heramind_core::llm::backend::FinishReason::Stop,
                                        usage: Some(heramind_core::llm::backend::TokenUsage::new(
                                            0, 0,
                                        )),
                                        tool_calls: None,
                                    })
        ...
        tokio::spawn(async move {
            ...
            while let Some((event, _metadata)) = rx.recv().await {
                ...
                if let heramind_core::HeraMindEvent::DeviceDiscovered {
                    device_id,
                    source,
                    adapter_id,
                    metadata,
    ...
    pub async fn get_or_init_agent_manager(
        ...
        let executor_config = heramind_agent::ai_agent::AgentExecutorConfig {
        ...
        tokio::spawn(async move {
            if let Ok(agents) = store
                .query_agents(heramind_storage::AgentFilter {
                    status: Some(heramind_storage::AgentStatus::Executing),
                    ..Default::default()
                })
                .await
            {
                for agent in agents {
                    ...
                    let _ = store
                        .update_agent_status(
                            &agent.id,
                            heramind_storage::AgentStatus::Active,
...
async fn apply_persisted_tool_disabled_state(
    runtime: &heramind_core::extension::ExtensionRuntime,
    tool_registry: &std::sync::Arc<heramind_agent::toolkit::ToolRegistry>,
) {
