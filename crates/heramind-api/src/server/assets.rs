...
<html>
<head>
    <title>HeraMind - Static Assets Not Available</title>
    <style>
...
pub async fn serve_index() -> impl IntoResponse {
    Assets::get("index.html")
        ...
        .unwrap_or_else(|| {
            let mut headers = HeaderMap::new();
            headers.insert(header::CONTENT_TYPE, HeaderValue::from_static("text/html"));
            (headers, "<!DOCTYPE html><html><body>HeraMind UI not found. Please run: cd web && npm run build</body></html>".to_string())
        })
        .into_response()
}
...
/// If `HERAMIND_WEB_DIR` contains frontend files, serves them via ServeDir.
...
{
    let web_dir =
        std::env::var("HERAMIND_WEB_DIR").unwrap_or_else(|_| "/var/www/heramind".to_string());
