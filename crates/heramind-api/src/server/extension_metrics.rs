...
pub struct ExtensionMetricsCollector {
    ...
    runtime: Arc<heramind_core::extension::ExtensionRuntime>,
    ...
    event_bus: Option<Arc<heramind_core::EventBus>>,
}
...
impl ExtensionMetricsCollector {
    ...
    pub fn with_event_bus(mut self, event_bus: Arc<heramind_core::EventBus>) -> Self {
    ...
    fn get_collect_interval(info: &heramind_core::extension::ExtensionRuntimeInfo) -> Option<u64> {
            ...
            for param in params {
                if param.name == "collect_interval" {
                    ...
                    if let Some(ref default) = param.default_value {
                        match default {
                            heramind_core::extension::system::ParamMetricValue::Integer(n) => {
                                return Some(*n as u64);
                            }
                            heramind_core::extension::system::ParamMetricValue::Float(f) => {
        ...
        for info in extensions {
            ...
            for metric_value in metric_values {
                ...
                let value = match metric_value.value {
                    heramind_core::extension::ParamMetricValue::Integer(n) => {
                        heramind_devices::mdl::MetricValue::Integer(n)
                    }
                    heramind_core::extension::ParamMetricValue::Float(f) => {
                        heramind_devices::mdl::MetricValue::Float(f)
                    }
                    heramind_core::extension::ParamMetricValue::String(s) => {
                        heramind_devices::mdl::MetricValue::String(s)
                    }
                    heramind_core::extension::ParamMetricValue::Boolean(b) => {
                        heramind_devices::mdl::MetricValue::Boolean(b)
                    }
                    _ => {
                        // Default to string for unknown types
                        heramind_devices::mdl::MetricValue::String(format!(
                            "{:?}",
                            metric_value.value
                        ))
                    }
                ...
                let data_point = heramind_devices::telemetry::DataPoint::new(timestamp, value);
                    ...
                    Ok(_) => {
                        ...
                        if let Some(ref bus) = self.event_bus {
                            let core_value = match &value_for_event {
                                heramind_devices::mdl::MetricValue::Integer(n) => {
                                    heramind_core::MetricValue::Integer(*n)
                                }
                                heramind_devices::mdl::MetricValue::Float(f) => {
                                    heramind_core::MetricValue::Float(*f)
                                }
                                heramind_devices::mdl::MetricValue::String(s) => {
                                    heramind_core::MetricValue::String(s.clone())
                                }
                                heramind_devices::mdl::MetricValue::Boolean(b) => {
                                    heramind_core::MetricValue::Boolean(*b)
                                }
                                other => heramind_core::MetricValue::String(format!("{:?}", other)),
                            };
                            bus.publish_sync(heramind_core::HeraMindEvent::ExtensionOutput {
