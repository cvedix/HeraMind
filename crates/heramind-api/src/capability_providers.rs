...
//! for the core capabilities provided by the HeraMind system.
//!
//! # Architecture
//!
...
use heramind_devices::{DeviceService, TimeSeriesStorage};
use heramind_rules::{RuleEngine, RuleId};
...
impl DeviceCapabilityProvider {
    ...
    async fn handle_metrics_write(&self, params: &Value) -> Result<Value, CapabilityError> {
        ...
        let data_point = heramind_devices::telemetry::DataPoint {
        ...
        if let Some(event_bus) = self.services.get::<EventBus>(keys::EVENT_BUS) {
            use heramind_core::event::MetricValue as EventMetricValue;
    ...
    async fn handle_template_register(&self, params: &Value) -> Result<Value, CapabilityError> {
        ...
        let template = heramind_devices::DeviceTypeTemplate {
            device_type: device_type.to_string(),
            name: name.to_string(),
            description,
            categories,
            mode: heramind_devices::DeviceTypeMode::Full,
            metrics,
            uplink_samples: vec![],
            commands,
    ...
    async fn handle_device_register(&self, params: &Value) -> Result<Value, CapabilityError> {
        ...
        let connection_config: heramind_devices::ConnectionConfig = params
        ...
        device_service
            .update_device_status(device_id, heramind_devices::ConnectionStatus::Connected)
            .await;
...
impl ExtensionCapabilityProvider for DeviceCapabilityProvider {
    fn capability_manifest(&self) -> CapabilityManifest {
        CapabilityManifest {
            ...
            package_name: "heramind-api::device".to_string(),
        }
    }
...
pub struct EventCapabilityProvider {
    ...
    event_dispatcher: Option<std::sync::Arc<heramind_core::extension::EventDispatcher>>,
}
...
impl EventCapabilityProvider {
    ...
    pub fn with_dispatcher(
        event_bus: Arc<EventBus>,
        event_dispatcher: std::sync::Arc<heramind_core::extension::EventDispatcher>,
    ) -> Self {
        Self {
            event_bus,
            subscriptions: std::sync::Arc::new(parking_lot::RwLock::new(
    ...
    fn handle_event_publish(&self, params: &Value) -> Result<Value, CapabilityError> {
        ...
        self.event_bus
            .publish_sync(heramind_core::event::HeraMindEvent::Custom {
...
impl ExtensionCapabilityProvider for EventCapabilityProvider {
    fn capability_manifest(&self) -> CapabilityManifest {
        CapabilityManifest {
            ...
            api_version: "v1".to_string(),
            min_core_version: env!("CARGO_PKG_VERSION").to_string(),
            package_name: "heramind-api::event".to_string(),
        }
    }
...
impl ExtensionCapabilityProvider for TelemetryCapabilityProvider {
    fn capability_manifest(&self) -> CapabilityManifest {
        CapabilityManifest {
            ...
            api_version: "v1".to_string(),
            min_core_version: env!("CARGO_PKG_VERSION").to_string(),
            package_name: "heramind-api::telemetry".to_string(),
        }
    }
...
impl ExtensionCapabilityProvider for RuleCapabilityProvider {
    fn capability_manifest(&self) -> CapabilityManifest {
        CapabilityManifest {
            ...
            api_version: "v1".to_string(),
            min_core_version: env!("CARGO_PKG_VERSION").to_string(),
            package_name: "heramind-api::rule".to_string(),
        }
    }
...
impl ExtensionCallCapabilityProvider {
    ...
    async fn handle_extension_call(&self, params: &Value) -> Result<Value, CapabilityError> {
        ...
        use heramind_core::extension::ExtensionRegistry;
...
impl ExtensionCapabilityProvider for ExtensionCallCapabilityProvider {
    fn capability_manifest(&self) -> CapabilityManifest {
        CapabilityManifest {
            ...
            api_version: "v1".to_string(),
            min_core_version: env!("CARGO_PKG_VERSION").to_string(),
            package_name: "heramind-api::extension".to_string(),
        }
    }
...
impl ExtensionCapabilityProvider for StorageCapabilityProvider {
    fn capability_manifest(&self) -> CapabilityManifest {
        CapabilityManifest {
            ...
            api_version: "v1".to_string(),
            min_core_version: env!("CARGO_PKG_VERSION").to_string(),
            package_name: "heramind-api::storage".to_string(),
        }
    }
...
use heramind_agent::ai_agent::AiAgentManager;
use heramind_storage::AgentStore;
...
impl AgentCapabilityProvider {
    ...
    async fn handle_agent_status(&self, params: &Value) -> Result<Value, CapabilityError> {
        ...
        if let Some(id) = agent_id {
            ...
            if let Some(manager) = &agent_manager {
                match manager
                    .list_agents(heramind_storage::AgentFilter::default())
                    .await
                {
                    Ok(agents) => {
                ...
                match store
                    .query_agents(heramind_storage::AgentFilter::default())
                    .await
                {
                    Ok(agents) => {
                        let agent_list: Vec<Value> = agents
...
impl ExtensionCapabilityProvider for AgentCapabilityProvider {
    fn capability_manifest(&self) -> CapabilityManifest {
        CapabilityManifest {
            ...
            api_version: "v1".to_string(),
            min_core_version: env!("CARGO_PKG_VERSION").to_string(),
            package_name: "heramind-api::agent".to_string(),
        }
    }
...
use heramind_agent::agent::types::AgentEvent;
...
/// `HeraMindEvent::AgentStreamChunk`, tagged with the session_id. The synchronous
...
impl ExtensionCapabilityProvider for ChatStreamCapabilityProvider {
    fn capability_manifest(&self) -> CapabilityManifest {
        CapabilityManifest {
            ...
            api_version: "v1".to_string(),
            min_core_version: env!("CARGO_PKG_VERSION").to_string(),
            package_name: "heramind-api::chat_stream".to_string(),
        }
    }
            ...
            let result = async {
                let stream = match mgr.process_message_events(&sid, &msg).await {
                    ...
                    Err(e) => {
                        ...
                        let _ = bus
                            .publish(HeraMindEvent::AgentStreamChunk {
                        ...
                        let _ = bus
                            .publish(HeraMindEvent::AgentStreamEnd {
                                session_id: sid.clone(),
                                reason: "error".into(),
...
impl ExtensionCapabilityProvider for ChatSessionCapabilityProvider {
    fn capability_manifest(&self) -> CapabilityManifest {
        CapabilityManifest {
            ...
            api_version: "v1".to_string(),
            min_core_version: env!("CARGO_PKG_VERSION").to_string(),
            package_name: "heramind-api::chat_session".to_string(),
        }
    }
...
impl ChatSessionCapabilityProvider {
    ...
    async fn handle_open(&self, params: &Value) -> Result<Value, CapabilityError> {
        ...
        let mut opts = heramind_agent::CreateSessionOptions::default();
...
use heramind_core::extension::ExtensionContext;
...
pub async fn register_builtin_providers_with_dispatcher(
    ...
    context
        .register_provider("heramind-api::device".to_string(), device_provider)
        .await;
    ...
    context
        .register_provider("heramind-api::event".to_string(), event_provider)
        .await;

    let telemetry_provider = Arc::new(TelemetryCapabilityProvider::new(services.clone()));
    context
        .register_provider("heramind-api::telemetry".to_string(), telemetry_provider)
        .await;

    let rule_provider = Arc::new(RuleCapabilityProvider::new(services.clone()));
    context
        .register_provider("heramind-api::rule".to_string(), rule_provider)
        .await;

    let extension_provider = Arc::new(ExtensionCallCapabilityProvider::new(services.clone()));
    context
        .register_provider("heramind-api::extension".to_string(), extension_provider)
        .await;

    let storage_provider = Arc::new(StorageCapabilityProvider::new(services.clone()));
    context
        .register_provider("heramind-api::storage".to_string(), storage_provider)
        .await;

    let agent_provider = Arc::new(AgentCapabilityProvider::new(services));
    context
        .register_provider("heramind-api::agent".to_string(), agent_provider)
        .await;
    ...
    context
        .register_provider(
            "heramind-api::chat_session".to_string(),
...
impl CompositeCapabilityProvider {
    ...
    pub fn with_all_providers(
        ...
        composite
            .providers
            .insert("heramind-api::device".to_string(), device_provider);
        ...
        composite
            .providers
            .insert("heramind-api::event".to_string(), event_provider);

        let telemetry_provider = Arc::new(TelemetryCapabilityProvider::new(services.clone()));
        composite
            .providers
            .insert("heramind-api::telemetry".to_string(), telemetry_provider);

        let rule_provider = Arc::new(RuleCapabilityProvider::new(services.clone()));
        composite
            .providers
            .insert("heramind-api::rule".to_string(), rule_provider);

        let extension_provider = Arc::new(ExtensionCallCapabilityProvider::new(services.clone()));
        composite
            .providers
            .insert("heramind-api::extension".to_string(), extension_provider);

        let storage_provider = Arc::new(StorageCapabilityProvider::new(services.clone()));
        composite
            .providers
            .insert("heramind-api::storage".to_string(), storage_provider);

        let agent_provider = Arc::new(AgentCapabilityProvider::new(services));
        composite
            .providers
            .insert("heramind-api::agent".to_string(), agent_provider);
...
impl ExtensionCapabilityProvider for CompositeCapabilityProvider {
    fn capability_manifest(&self) -> CapabilityManifest {
        ...
        CapabilityManifest {
            ...
            package_name: "heramind-api::composite".to_string(),
        }
    }
...
fn capability_to_provider(capability: &ExtensionCapability) -> &'static str {
    match capability {
        ...
        | ExtensionCapability::DeviceTemplateRegister
        | ExtensionCapability::DeviceRegister
        | ExtensionCapability::DeviceUnregister => "heramind-api::device",
        ...
        ExtensionCapability::TelemetryHistory | ExtensionCapability::MetricsAggregate => {
            "heramind-api::telemetry"
        }

        ExtensionCapability::RuleTrigger => "heramind-api::rule",

        ExtensionCapability::ExtensionCall => "heramind-api::extension",

        ExtensionCapability::StorageQuery => "heramind-api::storage",

        ExtensionCapability::AgentTrigger => "heramind-api::agent",

        ExtensionCapability::ChatStream => "heramind-api::chat_stream",

        ExtensionCapability::ChatStreamCancel => "heramind-api::chat_stream_cancel",

        ExtensionCapability::ChatSessionOpen
        | ExtensionCapability::ChatSessionSend
        | ExtensionCapability::ChatSessionClose
        | ExtensionCapability::ChatStreamCancelTurn => "heramind-api::chat_session",

        ExtensionCapability::Custom(_) => "heramind-api::custom",
    }
}
...
fn parse_metric_from_json(v: &Value) -> Option<heramind_devices::MdlMetricDefinition> {
    serde_json::from_value(v.clone()).ok()
}

/// Parse a CommandDefinition from a JSON object
fn parse_command_from_json(v: &Value) -> Option<heramind_devices::CommandDefinition> {
    serde_json::from_value(v.clone()).ok()
}
