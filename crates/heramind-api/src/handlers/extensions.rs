    ...
    for metric in &ext_info.metrics {
        ...
        if let Some(value) = metric_value {
            ...
            let core_value = match metric.data_type {
                heramind_core::extension::MetricDataType::Float => {
                    value.as_f64().map(CoreMetricValue::Float)
                }
                heramind_core::extension::MetricDataType::Integer => {
                    value.as_i64().map(CoreMetricValue::Integer)
                }
                heramind_core::extension::MetricDataType::Boolean => {
                    value.as_bool().map(CoreMetricValue::Boolean)
                }
                heramind_core::extension::MetricDataType::String => value
                    .as_str()
    ...
    for (name, value) in &metrics {
        ...
        if let Some(bus) = state.core.event_bus.as_ref() {
            let core_val = match value {
                MetricValue::Integer(n) => heramind_core::MetricValue::Integer(*n),
...
pub struct ExtensionRequirements {
    #[serde(default)]
    pub min_heramind_version: String,
    #[serde(default)]
    pub network: bool,
    #[serde(default)]
...
/// Uses String for category to be compatible with heramind-core's definition.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
fn load_extension_components(
    ...
    tracing::debug!(
        extension_id = %extension_id,
        data_dir = %data_dir,
        "Loading extension components (HERAMIND_DATA_DIR set)"
    );
