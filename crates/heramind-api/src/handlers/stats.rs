...
pub async fn get_system_stats_handler(
    ...
    let (device_stats_result, rule_stats_result, all_messages) = tokio::join!(
        async {
            ...
            use heramind_devices::adapter::ConnectionStatus;
            let online = state
                .devices
                .service
    ...
    let active_messages: Vec<_> = all_messages
        .into_iter()
        .filter(|m| matches!(m.status, heramind_messages::MessageStatus::Active))
        .collect();
    ...
    let alert_stats = AlertStats {
        ...
        by_severity: json!({
            "info": active_messages.iter().filter(|m| matches!(m.severity, heramind_messages::MessageSeverity::Info)).count(),
            "warning": active_messages.iter().filter(|m| matches!(m.severity, heramind_messages::MessageSeverity::Warning)).count(),
            "critical": active_messages.iter().filter(|m| matches!(m.severity, heramind_messages::MessageSeverity::Critical)).count(),
            "emergency": active_messages.iter().filter(|m| matches!(m.severity, heramind_messages::MessageSeverity::Emergency)).count(),
        }),
