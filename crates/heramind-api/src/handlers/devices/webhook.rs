...
use heramind_devices::adapters::webhook::WebhookPayload;
...
fn parse_multipart_body(
    ...
    tracing::info!(
        target: "heramind::api::webhook::multipart",
...
pub async fn webhook_handler(
    ...
    let metrics_count = adapter
        ...
        .map_err(|e| {
            ...
            match e {
                heramind_devices::adapter::AdapterError::Connection(msg) => {
                    ErrorResponse::unauthorized(msg)
                }
                heramind_devices::adapter::AdapterError::Configuration(msg) => {
...
pub async fn get_webhook_url_handler(
    ...
    if url_source == crate::handlers::common::ServerUrlSource::Fallback {
        payload["hint"] = serde_json::json!(
            "Set HERAMIND_SERVER_URL env var (or run behind a proxy that sends \
