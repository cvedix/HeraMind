...
use heramind_devices::EmbeddedBrokerConfig;
use heramind_storage::settings::MqttCredential;
...
pub struct AddCredentialRequest {
    /// Username (1-64 chars, cannot start with "__heramind")
    pub username: String,
...
/// Passwords are masked. Internal system credentials (starting with `__heramind`)
...
pub async fn get_broker_config_handler() -> HandlerResult<serde_json::Value> {
    ...
    let credentials: Vec<CredentialDto> = all_creds
        .into_iter()
        .filter(|c| !c.username.starts_with("__heramind"))
...
///
/// Validates username (1-64 chars, cannot start with `__heramind`),
...
pub async fn add_credential_handler(
    ...
    if req.username.starts_with("__heramind") {
        return Err(ErrorResponse::bad_request(
            "Username cannot start with '__heramind' (reserved for system use)".to_string(),
        ));
    }
...
/// System credentials (starting with `__heramind`) cannot be deleted via this API.
pub async fn delete_credential_handler(
    ...
    if username.starts_with("__heramind") {
        return Err(ErrorResponse::bad_request(
            "Cannot delete system credentials (starting with '__heramind')".to_string(),
        ));
    }
