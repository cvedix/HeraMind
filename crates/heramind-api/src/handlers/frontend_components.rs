...
use heramind_storage::frontend_components::{ComponentManifest, MarketIndex};
...
async fn publish_lifecycle_event(state: &ServerState, component_id: &str, lifecycle_state: &str) {
    if let Some(bus) = &state.core.event_bus {
        let _ = bus
            .publish(heramind_core::event::HeraMindEvent::Custom {
...
/// compromised marketplace URL (or `HERAMIND_MARKET_URL` env override) could
...
async fn fetch_market_index(client: &reqwest::Client) -> Result<MarketIndex, ErrorResponse> {
    ...
    let response = client
        .get(&index_url)
        .header("User-Agent", "HeraMind-Component-Marketplace")
...
pub async fn list_components_handler(
    ...
    let builtin_types: Vec<serde_json::Value> = heramind_core::dashboard::BUILTIN_WIDGET_TYPES
        .iter()
        .map(|t| {
            json!({
                "type_id": t.type_id,
...
fn builtin_component_list() -> Vec<ComponentManifest> {
    use heramind_storage::frontend_components::SizeConstraints;
