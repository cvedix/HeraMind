...
use heramind_core::llm::detect_vision_capability;
use heramind_storage::{BackendCapabilities, LlmBackendInstance, LlmBackendStore, LlmBackendType};
...
pub async fn update_capabilities_override_handler(
    ...
    if let Some(v) = req.multimodal {
        ...
        let detected = heramind_core::llm::detect_vision_capability(&instance.model);
        ...
        let source: Option<&str> =
            if heramind_core::llm::registry::lookup_vision(&instance.model).is_some() {
                Some("registry")
            } else if heramind_core::llm::registry::heuristic_vision_match(&instance.model) {
                Some("heuristic")
            } else {
                None
            };
...
fn detect_ollama_model_capabilities_from_name(model_name: &str) -> BackendCapabilities {
    ...
    let multimodal_source = if heramind_core::llm::registry::lookup_vision(model_name).is_some() {
        Some("registry".to_string())
    } else if heramind_core::llm::registry::heuristic_vision_match(model_name) {
...
fn adjust_capabilities_for_model(model_name: &str, capabilities: &mut BackendCapabilities) {
    ...
    // Use the same logic as heramind-core capability detection.
    ...
    if capabilities.multimodal_user_override.is_none() {
        ...
        if heramind_core::llm::registry::lookup_vision(&name_lower).is_some() {
            capabilities.multimodal_source = Some("registry".to_string());
        } else if heramind_core::llm::registry::heuristic_vision_match(&name_lower) {
