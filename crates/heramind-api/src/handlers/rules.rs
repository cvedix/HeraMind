...
use heramind_devices::MetricDataType as DeviceMetricDataType;
use heramind_rules::{
        ...
        RuleCondition::Comparison {
            ...
            let mut json = json!({
                ...
                "source_type": match source.source_type {
                    heramind_core::datasource::DataSourceType::Device => "device",
                    heramind_core::datasource::DataSourceType::Extension => "extension",
                    heramind_core::datasource::DataSourceType::Transform => "transform",
                },
        ...
        RuleAction::Execute {
            ...
            json!({
                ...
                "target_type": match target_type {
                    heramind_rules::ExecuteTarget::Device => "device",
                    heramind_rules::ExecuteTarget::Extension => "extension",
                },
                "command": command,
                "params": params,
            })
    ...
    fn from(rule: &CompiledRule) -> Self {
        ...
        let trigger_json = match &rule.trigger {
            RuleTrigger::DataChange { sources } => json!({
                ...
                "sources": sources.iter().map(|s: &heramind_core::datasource::DataSourceId| s.storage_key()).collect::<Vec<_>>(),
            }),
...
pub async fn update_rule_handler(
    ...
    if has_full_definition {
        ...
        // Validate rule against available resources
        {
            ...
            let result = heramind_rules::RuleValidator::validate_rule(
                &rule.condition,
                &rule.actions,
                &context,
            );
...
pub async fn test_rule_handler(
    ...
    let query_source = if source.source_type == heramind_core::datasource::DataSourceType::Device {
        ...
        heramind_core::datasource::DataSourceId::device(&device_id, &source.field_path)
    } else {
        source.clone()
    };
    ...
    if let Some(n) = input_value {
        current_value = Some(heramind_rules::RuleValue::Number(n));
        used_input_value = true;
    } else if let Some(s) = input_text.clone() {
        current_value = Some(heramind_rules::RuleValue::Text(s));
        used_input_value = true;
    }
    ...
    if current_value.is_none()
        && query_source.source_type == heramind_core::datasource::DataSourceType::Device
        && heramind_rules::VIRTUAL_METRICS.contains(&query_source.field_path.as_str())
        && query_source.field_path == "__last_seen_age_secs"
    {
        ...
        if last_seen > 0 {
            ...
            current_value = Some(heramind_rules::RuleValue::Number(metric_value));
        } else {
            return Err(ErrorResponse::bad_request(format!(
                ...
                "1. Verify the device is online and publishing: heramind device get <ID>\n\
                 2. Send a test data point: heramind device write-metric <ID> --metric <METRIC> --value <VALUE>\n\
    ...
    let telemetry_source_id = match query_source.source_type {
        heramind_core::datasource::DataSourceType::Device => {
        ...
        heramind_core::datasource::DataSourceType::Transform => {
    ...
    let mut telemetry_value: Option<heramind_rules::RuleValue> = None;
        ...
        match result {
            Ok(Some(point)) => {
                telemetry_value = match &point.value {
                    heramind_devices::MetricValue::Float(v) => {
                        Some(heramind_rules::RuleValue::Number(*v))
                    }
                    heramind_devices::MetricValue::Integer(v) => {
    ...
    let condition_met = if let Some(rv) = used_value.as_ref() {
        match condition {
            RuleCondition::Comparison { .. } => match rv {
                heramind_rules::RuleValue::Number(v) => operator.evaluate(*v, threshold),
                heramind_rules::RuleValue::Text(s) => {
        ...
        return Err(ErrorResponse::bad_request(
            ...
            "The data source must have produced data at least once before testing rules.\n\
             1. Check source status: heramind device get <ID> / heramind extension get <ID>\n\
             2. Send test data: heramind device write-metric <ID> --metric <METRIC> --value <VALUE>\n\
             3. Then retry: heramind rule test <RULE_ID>"
        ));
    };
...
pub async fn create_rule_handler(
    ...
    // non-existent devices/metrics/agents at creation time)
    {
        let context = build_validation_context(&state);
        let result =
            heramind_rules::RuleValidator::validate_rule(&rule.condition, &rule.actions, &context);
        if !result.is_valid {
            let detail = result
                .errors
                .iter()
...
pub async fn get_resources_handler(
    ...
    use heramind_devices::ConnectionStatus;
    use heramind_rules::{CommandInfo, DeviceInfo, MetricDataType, MetricInfo, ParameterInfo};
    ...
    let message_categories = vec![
        heramind_rules::AlertChannelInfo {
            id: "alert".to_string(),
...
fn convert_metric_data_type(dt: DeviceMetricDataType) -> heramind_rules::MetricDataType {
    match dt {
        DeviceMetricDataType::Integer => heramind_rules::MetricDataType::Number,
        DeviceMetricDataType::Float => heramind_rules::MetricDataType::Number,
        DeviceMetricDataType::String => heramind_rules::MetricDataType::String,
        DeviceMetricDataType::Boolean => heramind_rules::MetricDataType::Boolean,
        DeviceMetricDataType::Binary => heramind_rules::MetricDataType::String, // Binary as base64 string
        DeviceMetricDataType::Enum { .. } => heramind_rules::MetricDataType::Enum(vec![]),
        DeviceMetricDataType::Array { .. } => heramind_rules::MetricDataType::String, // Arrays as JSON string
    }
}
...
fn build_validation_context(state: &ServerState) -> heramind_rules::ValidationContext {
    use heramind_rules::{DeviceInfo, ValidationContext};
...
) -> Vec<heramind_rules::MetricInfo> {
    use heramind_devices::MetricDataType as DevicesMetricDataType;
    use heramind_rules::{MetricDataType, MetricInfo};
...
fn validate_virtual_metric_policy(rule: &CompiledRule) -> Result<(), ErrorResponse> {
    if let Err(msg) = heramind_rules::RuleValidator::validate_virtual_metric_cooldown(rule) {
...
fn validate_rule_cron(rule: &CompiledRule) -> Result<(), ErrorResponse> {
    if let heramind_rules::RuleTrigger::Schedule { ref cron } = rule.trigger {
...
pub async fn validate_rule_handler(
    ...
    use heramind_rules::{RuleCondition, RuleValidator};
