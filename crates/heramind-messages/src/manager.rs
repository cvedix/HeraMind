...
pub struct MessageManager {
    ...
    storage: Arc<RwLock<Option<Arc<heramind_storage::MessageStore>>>>,
    /// Notification channels
    channels: Arc<RwLock<ChannelRegistry>>,
    /// Optional event bus for publishing message events
    event_bus: Arc<RwLock<Option<Arc<heramind_core::EventBus>>>>,
...
impl MessageManager {
    ...
    pub fn with_storage<P: AsRef<Path>>(data_dir: P) -> Result<Self> {
        ...
        let store = Arc::new(heramind_storage::MessageStore::open(&db_path).map_err(|e| {
    ...
    fn stored_to_message(stored: heramind_storage::StoredMessage) -> Message {
        Message {
    ...
    fn message_to_stored(msg: &Message) -> heramind_storage::StoredMessage {
        heramind_storage::StoredMessage {
            id: msg.id.to_string(),
    ...
    pub async fn set_event_bus(&self, event_bus: Arc<heramind_core::EventBus>) {
    ...
    pub async fn acknowledge(&self, id: &MessageId) -> Result<()> {
        ...
        if let Some(event_bus) = self.event_bus.read().await.as_ref() {
            use heramind_core::HeraMindEvent;
            let _ = event_bus
                .publish(HeraMindEvent::MessageAcknowledged {
                    message_id: id.to_string(),
    ...
    pub async fn resolve(&self, id: &MessageId) -> Result<()> {
        ...
        if let Some(event_bus) = self.event_bus.read().await.as_ref() {
            use heramind_core::HeraMindEvent;
            let _ = event_bus
                .publish(HeraMindEvent::MessageResolved {
                    message_id: id.to_string(),
                    timestamp: chrono::Utc::now().timestamp(),
                })
                .await;
    ...
    pub async fn acknowledge_multiple(&self, ids: &[MessageId]) -> Result<usize> {
        let (count, to_persist): (usize, Vec<heramind_storage::StoredMessage>) = {
            let mut messages = self.messages.write().await;
            let mut count = 0;
    ...
    async fn test_system_message_creation() {
        ...
        let msg = manager
            .system_message(
                ...
                "HeraMind system initialized successfully".to_string(),
            )
            .await
            .unwrap();
