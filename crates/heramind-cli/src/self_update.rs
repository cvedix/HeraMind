...
//! swap when `/var/www/heramind` exists.
//!
...
pub async fn run_upgrade(version: Option<String>, yes: bool) -> Result<()> {
    ...
    println!("HeraMind {} — checking for updates...", APP_VERSION);
    ...
    let cur_bin = std::env::current_exe()
        ...
        .canonicalize()
        .unwrap_or_else(|_| PathBuf::from(DEFAULT_INSTALL_DIR).join("heramind"));
    ...
    if svc_active {
        ...
        let _ = sh(sudo.as_deref(), &["systemctl", "stop", "heramind"]);
    }

    // 7. Back up the current binaries for rollback.
    let bak_bin = install_dir.join(format!("heramind.{}.bak", APP_VERSION));
    let bak_runner = install_dir.join(format!("heramind-extension-runner.{}.bak", APP_VERSION));
    if cur_bin.exists() {
            ...
            if resp.status().is_success() {
                if let Ok(bytes) = resp.bytes().await {
                    ...
                    if std::fs::write(&web_tgz, &bytes).is_ok() {
                        ...
                        let _ = sh(
                            sudo.as_deref(),
                            &["chown", "-R", "heramind:heramind", &stage],
                        );
    ...
    if svc_active {
        ...
        let _ = sh(sudo.as_deref(), &["systemctl", "start", "heramind"]);
    }
    ...
    if svc_active {
        let active = Command::new("systemctl")
            ...
            .unwrap_or(false);
        if active {
            println!("✅ heramind upgraded to v{} (service active)", target);
        } else {
            eprintln!("⚠️ service did not come back up — check: sudo systemctl status heramind");
        }
    } else {
        println!(
            "✅ heramind upgraded to v{} (no systemd service detected — restart manually)",
            target
        );
    }
    println!(
...
pub async fn run_uninstall(purge: bool, yes: bool) -> Result<()> {
    ...
    let _ = sh(sudo.as_deref(), &["systemctl", "stop", "heramind"]);
    let _ = sh(sudo.as_deref(), &["systemctl", "disable", "heramind"]);
    ...
    let cur_bin = std::env::current_exe()
        ...
        .unwrap_or_else(|| PathBuf::from(format!("{}/heramind", DEFAULT_INSTALL_DIR)));
    let dir = cur_bin
        .parent()
