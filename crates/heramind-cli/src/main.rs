//! Command-line interface for HeraMind.
...
use anyhow::Result;
use clap::Parser;
use heramind_agent::{LlmBackend, SessionManager};
...
async fn main() -> Result<()> {
    ...
    heramind_core::extension::safety::install_extension_panic_hook();

    let args = Args::parse();

    // Initialize logging
    let json_logging = std::env::var("HERAMIND_LOG_JSON")
        .ok()
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if args.verbose {
            tracing_subscriber::EnvFilter::new("heramind=debug")
                .add_directive(tracing::Level::DEBUG.into())
        } else {
            tracing_subscriber::EnvFilter::new("heramind=info")
...
fn print_result(
    result: anyhow::Result<(
        heramind_cli_ops::types::CliResponse,
        heramind_cli_ops::types::OutputFormat,
    )>,
) -> Result<()> {
    let (resp, fmt) = result?;
...
async fn run_prompt(prompt: &str) -> Result<()> {
    println!("HeraMind Edge AI - Prompt Mode");
    println!("==============================\n");
...
async fn run_chat(session_id: Option<String>) -> Result<()> {
    println!("HeraMind Edge AI - Chat Mode");
    println!("===========================\n");
            ...
            .await
        {
            Ok(mut stream) => {
                ...
                while let Some(event) = stream.next().await {
                    match event {
                        heramind_agent::AgentEvent::Thinking { content } => {
                            if !content.is_empty() {
                        ...
                        heramind_agent::AgentEvent::Content { content } => {
                        ...
                        heramind_agent::AgentEvent::ToolCallStart { tool, .. } => {
                            println!("\n\n[Calling tool: {}]", tool);
                        }
                        heramind_agent::AgentEvent::ToolCallEnd { tool, success, .. } => {
                            if success {
                        ...
                        heramind_agent::AgentEvent::Error { message } => {
                            eprintln!("\nError: {}", message);
                        }
                        heramind_agent::AgentEvent::Intent {
                            display_name,
                            confidence,
                            ..
                        } => {
                        ...
                        heramind_agent::AgentEvent::Plan { step, stage } => {
                            println!("[Plan: {} - {}]", stage, step);
                        }
                        heramind_agent::AgentEvent::Progress { message, .. } => {
                            println!("[Progress: {}]", message);
                        }
                        heramind_agent::AgentEvent::Heartbeat { .. } => {
                            // Ignore heartbeat events in CLI
                        }
                        heramind_agent::AgentEvent::Warning { message } => {
                            eprintln!("[Warning] {}", message);
                        }
                        heramind_agent::AgentEvent::IntermediateEnd => {
                            // Intermediate end - more content coming, don't break
                            println!("[Continuing...]");
                        }
                        heramind_agent::AgentEvent::End { .. } => {
                            break;
                        }
                    }
                }
        ...
        for entry in entries.flatten() {
            ...
            let should_remove = if let Some(filename) = path.file_name().and_then(|n| n.to_str()) {
                if let Some(date_str) = filename.strip_prefix("heramind.log.") {
                    // Parse date from filename: heramind.log.YYYY-MM-DD
                    ...
                    // Not a rotated log file (e.g., current heramind.log), skip
                    false
                }
            } else {
                false
...
async fn run_health() -> Result<()> {
    println!("HeraMind System Health Check");
    println!("==========================\n");
    ...
    match reqwest::get(&check_url).await {
        ...
        Err(e) => {
            ...
            println!("  Hint: Start the server with 'heramind serve'");
        }
    }
    ...
    let api_client = heramind_cli_ops::ApiClient::new();
    match heramind_cli_ops::llm::list_backends(&api_client).await {
        Ok(response) => {
            if response.success {
                if let Some(data) = &response.data {
                    ...
                    if count > 0 {
                        ...
                        if let Some(id) = active_id {
                            ...
                            println!(
                                "  Hint: Use 'heramind llm activate <id>' to activate a backend"
                            );
                        }
                    } else {
                        println!("  ⚠️  No LLM backend configured");
                        println!("  Hint: Use 'heramind llm add' to add an LLM backend");
                    }
                }
            } else {
    ...
    let log_dir = log_dirs.iter().find(|d| d.exists()).ok_or_else(|| {
        ...
        anyhow::anyhow!(
            "Log directory not found. Searched in:\n{}\n\
                 Hint: Start the server with 'heramind serve' or run the Tauri desktop app.",
            paths
        )
    })?;
...
fn find_newest_log(log_dir: &Path) -> Result<std::path::PathBuf> {
    ...
    for entry in std::fs::read_dir(log_dir)? {
        ...
        // Only consider heramind log files
...
async fn show_extension_info(id_or_path: &str) -> Result<()> {
    use heramind_cli_ops::output::format_output;
    use heramind_cli_ops::types::OutputFormat;
...
fn create_extension_scaffold(
    ...
    let crate_name = format!("heramind_{}", name.replace('-', "_"));
    // Strip optional "heramind-" prefix for struct name, then PascalCase
    let struct_base = name.strip_prefix("heramind-").unwrap_or(name);
...
[dependencies]
heramind-extension-sdk = "{sdk_version}"
async-trait = "0.1"
serde_json = "1.0"
...
use heramind_extension_sdk::{{
    heramind_export, Extension, ExtensionCommand, ExtensionError, ExtensionMetadata,
    ...
    fn metadata(&self) -> &ExtensionMetadata {{
        ...
        META.get_or_init(|| {{
            ExtensionMetadata::new("{name}", "{struct_name}", "0.1.0")
                .with_description("A HeraMind extension")
        }})
    }}
...
"#,
    ...
    let manifest = serde_json::json!({
        "format": "heramind-extension-package",
        "abi_version": 3,
        "type": "native",
        "id": name,
        "name": struct_name,
    ...
    println!("  cd {}", name);
    println!("  cargo build");
    println!("  heramind extension build ./{}", name);
...
fn build_extension(path: &std::path::PathBuf) -> Result<()> {
    ...
    let manifest = if manifest_path.exists() {
        ...
        anyhow::bail!(
            "manifest.json not found at {}. Run 'heramind extension create' to scaffold a project first.",
    ...
    let platform = heramind_core::extension::package::detect_platform();
