# HeraMind — Edge AI Platform for IoT
...
The spine is an async **EventBus** (`heramind_core::eventbus`) — all subsystems (devices, agents, rules, dashboards, frontend WS/SSE) communicate via pub/sub events, not direct calls.

## Development Commands

```bash
cargo build && cargo test && cargo run -p heramind-cli -- serve   # API on :9375, swagger at /api/docs
...
HeraMind is the **core platform**; three companion repos hold community content. This repo defines the **format contracts** each must follow.

| Repo | Contains | Loaded by this repo as |
|------|----------|------------------------|
| **[HeraMind](https://github.com/cvedix/HeraMind)** (this) | Backend + frontend + desktop app | — |
| **[HeraMind-Extensions](https://github.com/camthink-ai/HeraMind-Extensions)** | Official extension marketplace (weather, YOLO, OCR, face, HA/LoRaWAN/Modbus bridges, stream player…) | `.nep` packages → `data/extensions/<id>/` |
| **[HeraMind-DeviceTypes](https://github.com/camthink-ai/HeraMind-DeviceTypes)** | Standardized device type JSON templates (e.g. NE301, NE101 cameras) | JSON imported into the device registry |
| **[HeraMind-Dashboard-Components](https://github.com/camthink-ai/HeraMind-Dashboard-Components)** | Community dashboard widget marketplace (charts, gauges, maps…) | Bundles → `data/frontend-components/<id>/` |

### Extension package contract (`.nep`)
ZIP archive, parsed by `crates/heramind-core/src/extension/package.rs`. Install: `POST /api/extensions/install` or `heramind extension install file.nep`.

```
{extension-id}-{version}.nep
├── manifest.json                # format = "heramind-extension-package"
...
- **SDK**: `crates/heramind-extension-sdk/` is public API — never auto-strip its re-exports. Authors use `heramind_export!()` + `ExtensionMetadata::new()`.
...
- **Runtime**: process-isolated child process via `heramind-extension-runner`. Crash-loop detection auto-disables misbehaving extensions. Metrics → DataSources (`extension:<id>:<metric>`).
- **Hardware variant keys (CUDA/Jetson)**: variant discrimination lives **only** in marketplace `metadata.json` `builds` keys and release filenames — the `.nep` internal `manifest.binaries` key is always the plain OS+arch (e.g. `linux_arm64`), identical for CPU and Jetson builds. Download-side selection (`accel.rs::detect_variant` + `select_build_key`) resolves `linux-aarch64-jetson` → `linux-aarch64` → `wasm`. Detection: `/etc/nv_tegra_release` (Jetson, checked first) > `nvidia-smi` (CUDA) > CPU. Override with `HERAMIND_EXTENSION_VARIANT=cpu|cuda|jetson`. Adding a jetson build = upload `xxx-linux_arm64-jetson.nep` (standard internal structure) + add `linux-aarch64-jetson` entry to `metadata.json` `builds`. Do NOT add variant suffixes inside `.nep` `binaries/`.

### Device type template contract (JSON)
Maps to `DeviceTypeTemplate` (`crates/heramind-devices/src/registry.rs:159`). Required: `device_type` (unique id), `name`. Optional: `categories`, `mode` (`Simple` default), `metrics`, `commands`, `uplink_samples`, `default_offline_timeout_secs`.
...
2. **`web/src-tauri/` is NOT in the cargo workspace**. It imports `edge_api::start_server()` (alias for `heramind_api`). `cargo build` at workspace root will NOT catch breakage there — when refactoring `heramind-api` public re-exports, also grep `web/src-tauri/src/`.
3. **Multimodal capability priority**: `user override > runtime API detection (/api/show) > LiteLLM registry > heuristic > false`. Never let runtime detection clobber a user override. Single entry point: `supports_multimodal()` in `crates/heramind-agent/src/llm.rs`.
...
|-----------|-----------|
| Event bus types | `crates/heramind-core/src/event.rs` (`EventType` enum) |
| HTTP routes | `crates/heramind-api/src/server/router.rs` |
| App state / store wiring | `crates/heramind-api/src/server/types.rs` |
| Agent enums (`ScheduleType`/`ExecutionMode`/`AgentStatus`) | `crates/heramind-storage/src/agents.rs` |
| LLM backend capabilities | `crates/heramind-core/src/llm/capability.rs` + `registry.rs` |
| Multimodal image parsing | `crates/heramind-agent/src/image_utils.rs` (single source of truth) |
...
| Extension SDK public surface | `crates/heramind-extension-sdk/` (public API — never auto-strip re-exports) |
...
The LLM's `shell` tool intercepts `heramind ...` commands and dispatches them **in-process** via `heramind_cli_ops::dispatch::dispatch(argv)` — no subprocess. `dispatch()` uses `try_parse_from` (bad args → `Parse` error, not `exit()`). Falls back to subprocess only for `Serve | Prompt | Chat | Logs | Health`. Failed commands return a `CliResponse` with `suggestion: Option<String>` recovery hints.
...
**Telemetry → dashboard**: device → MQTT → `heramind-devices` parses → writes `telemetry.redb` → publishes `DeviceMetric` on EventBus → WS/SSE → frontend `deviceSlice` → widget re-renders.
...
- **Dead re-export detection**: grep has a blind spot for multi-line brace imports (`heramind_crate::{\n TypeA,\n TypeB}`). Compiler-based is reliable: (1) strip all re-exports to empty, (2) `cargo build --tests 2>&1 | grep "unresolved import"`, (3) add back exactly what the compiler reports. Skip this for `heramind-extension-sdk` (public API).
