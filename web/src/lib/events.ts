...
//
// Provides real-time event streaming from the HeraMind event bus.
...
export interface HeraMindEvent {
  id: string
  type: EventType
  timestamp: number
  source: string
...
export interface DeviceMetricEvent extends HeraMindEvent {
  type: 'DeviceMetric'
  data: {
    device_id: string
    metric: string
...
export interface ExtensionOutputEvent extends HeraMindEvent {
...
export interface ExtensionLifecycleEvent extends HeraMindEvent {
...
export interface FrontendComponentLifecycleEvent extends HeraMindEvent {
...
export interface RuleTriggeredEvent extends HeraMindEvent {
  type: 'RuleTriggered'
  data: {
    rule_id: string
    rule_name: string
...
export interface AgentExecutionStartedEvent extends HeraMindEvent {
...
export interface AgentExecutionCompletedEvent extends HeraMindEvent {
...
export interface AgentThinkingEvent extends HeraMindEvent {
  type: 'AgentThinking'
  data: {
    agent_id: string
    execution_id: string
...
export interface AgentDecisionEvent extends HeraMindEvent {
  type: 'AgentDecision'
  data: {
    agent_id: string
    execution_id: string
...
export interface AgentProgressEvent extends HeraMindEvent {
  type: 'AgentProgress'
  data: {
    agent_id: string
    execution_id: string
...
export interface AgentMemoryUpdatedEvent extends HeraMindEvent {
...
export type EventHandler = (event: HeraMindEvent) => void
...
/**
 * EventsWebSocket - Manages real-time event streaming from HeraMind event bus
 *
...
export class EventsWebSocket {
  ...
  private pendingEvents: HeraMindEvent[] = []
    ...
    this.ws.onmessage = (event) => {
      try {
        ...
        if (this.authenticated) {
          this.notifyEvent(data as HeraMindEvent)
        }
      } catch {
  ...
  private connectSSE(category: string, eventTypes: EventType[]) {
    ...
    this.eventSource.onmessage = (event) => {
      try {
        const data = JSON.parse(event.data) as HeraMindEvent
   ...
   */
  on<T extends EventType>(eventType: T, handler: (event: Extract<HeraMindEvent, { type: T }>) => void) {
  ...
  private notifyEvent(event: HeraMindEvent) {
    ...
    if ((event as any).batch === true && (event as any).events) {
      ...
      const batchedEvents = (event as any).events as HeraMindEvent[]
