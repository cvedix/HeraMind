 ...
 * Instance Slice
 *
 * Manages remote HeraMind backend instances with CRUD operations,
  ...
  if (!pending) {
    ...
    try {
      ...
      if (currentId && currentId !== 'local-default') {
        const hasJwt = !!localStorage.getItem('heramind_token')
        const activeKey = sessionStorage.getItem('heramind_api_key')
        if (!hasJwt && !activeKey) {
