  ...
  setItem: (name: string, value: string): void => {
    try {
      ...
      if (error instanceof DOMException && error.name === 'QuotaExceededError') {
        ...
        localStorage.removeItem('heramind-store')
        try {
          localStorage.setItem(name, value)
        } catch (retryError) {
...
export type HeraMindStore = AuthSlice
  & SessionSlice
  & UISlice
  & AlertSlice
  & DeviceSlice
...
export const useStore = create<HeraMindStore>()(
  devtools(
    persist(
      (set, get, api) => ({
        // Combine all slices
...
if (typeof window !== 'undefined' && import.meta.env.DEV) {
  (window as any).heramindStore = useStore
}
