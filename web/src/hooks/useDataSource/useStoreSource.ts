...
import type { HeraMindStore } from '@/store'
...
export function useStoreSource<T = unknown>(
  ...
  const prevStoreStateRef = useRef<{
    rawDevices: HeraMindStore['devices']
    deviceTelemetry: HeraMindStore['deviceTelemetry']
  ...
  useEffect(() => {
    ...
    const processStoreChange = (s: HeraMindStore) => {
    ...
    const unsubscribe = useStore.subscribe((s: HeraMindStore) => {
