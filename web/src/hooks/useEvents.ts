...
import type { HeraMindEvent, EventType, EventCategory } from '@/lib/events'
...
export interface UseEventsResult {
   ...
   * Get events by type
   */
  getEventsByType: <T extends EventType>(type: T) => Extract<HeraMindEvent, { type: T }>[]
}

const DEFAULT_MAX_EVENTS = 100

/**
 * useEvents - Hook for subscribing to real-time HeraMind events
 *
...
export function useEvents(options: UseEventsOptions = {}): UseEventsResult {
  ...
  const [events, setEvents] = useState<HeraMindEvent[]>([])
  ...
  const getEventsByType = useCallback(<T extends EventType>(type: T) => {
    return events.filter(e => e.type === type) as Extract<HeraMindEvent, { type: T }>[]
  }, [events])
  ...
  useEffect(() => {
    ...
    import('@/lib/events').then((eventsLib) => {
      ...
      // Subscribe to events
      unsubscribeEvent = connection.onEvent((event: HeraMindEvent) => {
        if (isMounted) {
