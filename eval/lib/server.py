"""TestServer: spawn `heramind serve` per case with full isolation.
...
- Agent under test runs INSIDE the `heramind serve` subprocess via the
...
- Auth: we run `heramind api-key create` BEFORE spawning the server to
...
def _resolve_heramind_bin() -> str:
    """Locate the heramind binary.

    Priority:
    1. $HERAMIND_TEST_BIN override
    2. <cwd>/target/release/heramind (workspace's freshly-built binary —
       preferred over PATH because installed `heramind` is often stale; e.g.
       v0.8.11 on PATH while repo is at v0.9.0 caused silent API drift)
    3. `heramind` on PATH (last resort)
    """
    env = os.environ.get("HERAMIND_TEST_BIN")
    if env:
        return env
    cwd = os.getcwd()
    candidate = os.path.join(cwd, "target", "release", "heramind")
    if os.path.exists(candidate):
        _warn_if_stale(candidate)
        return candidate
    return "heramind"  # fall back to PATH
...
def _warn_if_stale(bin_path: str) -> None:
    ...
    hard-to-miss warning when the binary is older. Set HERAMIND_SKIP_STALE_CHECK=1
    to suppress (e.g. for prod binaries built elsewhere).
    """
    if os.environ.get("HERAMIND_SKIP_STALE_CHECK"):
        return
    try:
    ...
    if newest_src > bin_mtime:
        ...
        print(
            "⚠️  STALE BINARY WARNING: target/release/heramind was built at "
            f"{bin_t} but {newest_path} was modified at {src_t}.\n"
            "    Recent source changes are NOT in the binary the eval will "
            "run against.\n"
            "    Rebuild with `cargo build --release -p heramind-cli` before "
            "trusting eval results,\n"
            "    or set HERAMIND_SKIP_STALE_CHECK=1 to silence this warning.",
...
def _precreate_api_key(data_dir: Path) -> str:
    """Run `heramind api-key create` to seed a known plaintext key.
    ...
    bin_path = _resolve_heramind_bin()
...
class TestServer:
    """A per-case `heramind serve` subprocess with a temp data dir.
    ...
    def spawn(self, startup_timeout: float = 30.0) -> "TestServer":
        ...
        # 3. Spawn `heramind serve` with CWD=tmpdir so `AuthState::new()`'s
        #    hardcoded `data/api_keys.redb` lands inside tmpdir/data — same
        #    file we just seeded. Pass HERAMIND_API_BASE so in-process shell
        #    dispatch (CLAUDE.md "CLI In-Process Dispatch") inside the agent
        #    talks back to this same server.
        bin_path = _resolve_heramind_bin()
        env = os.environ.copy()
        env["HERAMIND_DATA_DIR"] = str(tmpdir_path)
        ...
        self.process = subprocess.Popen(
            ...
            # Kill the whole process group on shutdown — heramind spawns
        ...
        while time.monotonic() < deadline:
            if self.process.poll() is not None:
                raise RuntimeError(
                    f"heramind serve exited early (rc={self.process.returncode})"
                )
            try:
    ...
    def get_history(self, session_id: str) -> list:
        ...
        doesn't expose — see crates/heramind-agent/src/agent/types.rs:321.
        """
        r = requests.get(
