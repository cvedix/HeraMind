# HeraMind Chat Agent Eval System

A Claude-as-judge eval harness for the HeraMind chat AI agent. Python runner
spawns a real `heramind serve` subprocess per case (full isolation), drives the
...
│   ├── server.py               # spawn heramind serve, configure LLM, HTTP chat
...
The agent under test runs INSIDE the `heramind serve` subprocess via the
...
`heramind` binary itself.

Per case:
1. Spawn `heramind serve --host 127.0.0.1 --port <free>` with temp
   `HERAMIND_DATA_DIR` and CWD = tempdir.
2. Pre-seed a known API key via `heramind api-key create` BEFORE spawning —
...
3. Pass `HERAMIND_API_BASE` + `HERAMIND_API_KEY` as env on the subprocess so
...
# 1. Build the heramind binary (release; the runner uses target/release/heramind)
cargo build -p heramind-cli --release
...
| `HERAMIND_TEST_BIN` | Override `heramind` binary path (default `<cwd>/target/release/heramind`) | Runner process only |

The test server auto-derives its OWN API key — never set `HERAMIND_API_KEY`
yourself; the runner pre-seeds one via `heramind api-key create` and propagates
...
- **Pre-seeded `data/api_keys.redb` quirk**: `AuthState::new()` hardcodes the
  relative path `data/api_keys.redb` (does NOT respect `HERAMIND_DATA_DIR`).
